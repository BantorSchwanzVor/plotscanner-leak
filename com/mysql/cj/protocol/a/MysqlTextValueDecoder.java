...
public class MysqlTextValueDecoder implements ValueDecoder {
  ...
  public static boolean isTime(String s) {
    return s.contains(".") ? ((s
      .length() >= 8 && s.length() <= 17 && s.charAt(2) == ':' && s.charAt(5) == ':')) : ((s
      .length() >= 8 && s.length() <= 10 && s.charAt(2) == ':' && s.charAt(5) == ':'));
  }
  
  public static boolean isTimestamp(String s) {
    return (s.length() >= 19 && (s.length() <= 26 || s.length() == 29) && s
      .charAt(4) == '-' && s.charAt(7) == '-' && s.charAt(10) == ' ' && s.charAt(13) == ':' && s.charAt(16) == ':');
  }
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\com\mysql\cj\protocol\a\MysqlTextValueDecoder.class
