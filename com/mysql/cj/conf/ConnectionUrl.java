...
public abstract class ConnectionUrl implements DatabaseUrlContainer {
  ...
  public HostInfo getHostOrSpawnIsolated(String hostPortPair, List<HostInfo> hostsList) {
    ...
    return buildHostInfo(host, port.intValue(), user, password, true, this.properties);
  }
  ...
  private List<HostInfo> srvRecordsToHostsList(List<DnsSrv.SrvRecord> srvRecords, HostInfo baseHostInfo) {
    return (List<HostInfo>)srvRecords.stream().map(s -> buildHostInfo(s.getTarget(), s.getPort(), baseHostInfo.getUser(), baseHostInfo.getPassword(), baseHostInfo.isPasswordless(), baseHostInfo.getHostProperties()))
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\com\mysql\cj\conf\ConnectionUrl.class
