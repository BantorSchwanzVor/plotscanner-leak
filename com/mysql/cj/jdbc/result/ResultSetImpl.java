...
public class ResultSetImpl extends NativeResultset implements ResultSetInternalMethods, WarningListener {
  ...
  public ResultSetImpl(ResultsetRows tuples, JdbcConnection conn, StatementImpl creatorStmt) throws SQLException {
    ...
    this.defaultDateValueFactory = (ValueFactory<Date>)new SqlDateValueFactory((PropertySet)jdbcPropertySet, null, this.session.getServerSession().getDefaultTimeZone(), this);
    ...
    this.defaultTimestampValueFactory = (ValueFactory<Timestamp>)new SqlTimestampValueFactory((PropertySet)jdbcPropertySet, null, this.session.getServerSession().getDefaultTimeZone());
  ...
  public Date getDate(int columnIndex) throws SQLException {
    try {
      checkRowPos();
      checkColumnBounds(columnIndex);
      return (Date)this.thisRow.getValue(columnIndex - 1, this.defaultDateValueFactory);
    } catch (CJException cJException) {
      throw SQLExceptionsMapping.translateException(cJException, getExceptionInterceptor());
    } 
  }
  ...
  public Date getDate(int columnIndex, Calendar cal) throws SQLException {
    try {
      checkRowPos();
      checkColumnBounds(columnIndex);
      SqlDateValueFactory sqlDateValueFactory = new SqlDateValueFactory(this.session.getPropertySet(), cal, (cal != null) ? cal.getTimeZone() : this.session.getServerSession().getDefaultTimeZone(), this);
      return (Date)this.thisRow.getValue(columnIndex - 1, (ValueFactory)sqlDateValueFactory);
    } catch (CJException cJException) {
      throw SQLExceptionsMapping.translateException(cJException, getExceptionInterceptor());
    } 
  }
  ...
  public String getString(int columnIndex) throws SQLException {
    try {
      ...
      if (this.padCharsWithSpace && stringVal != null && f.getMysqlTypeId() == 254) {
        int maxBytesPerChar = this.session.getServerSession().getMaxBytesPerChar(Integer.valueOf(f.getCollationIndex()), f.getEncoding());
  ...
  public Timestamp getTimestamp(int columnIndex, Calendar cal) throws SQLException {
    try {
      ...
      if (this.customTsVf != null && tz == this.lastTsCustomTz)
        return (Timestamp)this.thisRow.getValue(columnIndex - 1, this.customTsVf); 
      SqlTimestampValueFactory sqlTimestampValueFactory = new SqlTimestampValueFactory(this.session.getPropertySet(), cal, tz);
      this.lastTsCustomTz = tz;
      this.customTsVf = (ValueFactory<Timestamp>)sqlTimestampValueFactory;
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\com\mysql\cj\jdbc\result\ResultSetImpl.class
