...
public class ResultSetMetaData implements ResultSetMetaData {
  ...
  public String getColumnCharacterSet(int column) throws SQLException {
    ...
    if ((((NativeSession)this.session).getProtocol().getServerSession()).indexToCustomMysqlCharset != null)
      charsetName = (String)(((NativeSession)this.session).getProtocol().getServerSession()).indexToCustomMysqlCharset.get(Integer.valueOf(index)); 
    if (charsetName == null)
      charsetName = CharsetMapping.getMysqlCharsetNameForCollationIndex(Integer.valueOf(index)); 
    return charsetName;
  }
  ...
  public int getColumnDisplaySize(int column) throws SQLException {
    try {
      ...
      return lengthInBytes / this.session.getServerSession().getMaxBytesPerChar(Integer.valueOf(f.getCollationIndex()), f.getEncoding());
    } catch (CJException cJException) {
      throw SQLExceptionsMapping.translateException(cJException, this.exceptionInterceptor);
    } 
  }
  ...
  public int getPrecision(int column) throws SQLException {
    try {
      ...
      return f.getMysqlType().isDecimal() ? clampedGetLength(f) : (
        clampedGetLength(f) / this.session.getServerSession().getMaxBytesPerChar(Integer.valueOf(f.getCollationIndex()), f.getEncoding()));
    } catch (CJException cJException) {
      throw SQLExceptionsMapping.translateException(cJException, this.exceptionInterceptor);
    } 
  }
  ...
  public boolean isCaseSensitive(int column) throws SQLException {
    try {
      ...
      switch (field.getMysqlType()) {
        ...
        case ENUM:
        case SET:
          collationName = CharsetMapping.COLLATION_INDEX_TO_COLLATION_NAME[field.getCollationIndex()];
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\com\mysql\cj\jdbc\result\ResultSetMetaData.class
