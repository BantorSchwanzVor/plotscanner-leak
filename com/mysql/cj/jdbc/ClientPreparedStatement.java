...
public class ClientPreparedStatement extends StatementImpl implements JdbcPreparedStatement {
    ...
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        rs = executeInternal(this.maxRows, sendPacket, createStreamingResultSet(), 
            (((PreparedQuery)this.query).getParseInfo().getFirstStmtChar() == 'S'), (ColumnDefinition)cachedMetadata, false);
  ...
  protected ClientPreparedStatement prepareBatchedInsertSQL(JdbcConnection localConn, int numBatches) throws SQLException {
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ClientPreparedStatement pstmt = new ClientPreparedStatement(localConn, "Rewritten batch of: " + ((PreparedQuery)this.query).getOriginalSql(), getCurrentDatabase(), ((PreparedQuery)this.query).getParseInfo().getParseInfoForBatch(numBatches));
    ...
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        if (this.pstmtResultMetaData == null)
          try {
            mdStmt = new ClientPreparedStatement(this.connection, ((PreparedQuery)this.query).getOriginalSql(), getCurrentDatabase(), ((PreparedQuery)this.query).getParseInfo());
            mdStmt.setMaxRows(1);
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\com\mysql\cj\jdbc\ClientPreparedStatement.class
