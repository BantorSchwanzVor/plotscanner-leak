...
public class ShadersRender {
  ...
  public static void renderShadowMap(EntityRenderer entityRenderer, int pass, float partialTicks, long finishTimeNano) {
    if (Shaders.usedShadowDepthBuffers > 0 && --Shaders.shadowPassCounter <= 0) {
      ...
      ActiveRenderInfo.updateRenderInfo((EntityPlayer)minecraft.player, (minecraft.gameSettings.thirdPersonView == 2));
      ...
      renderglobal.setupTerrain(entity, partialTicks, (ICamera)frustum, i, minecraft.player.isSpectator());
  ...
  public static boolean renderEndPortal(TileEntityEndPortal te, double x, double y, double z, float partialTicks, int destroyStage, float offset) {
    ...
    if (te.shouldRenderFace(EnumFacing.UP)) {
      bufferbuilder.pos(x, y + offset, z + 1.0D).color(f1, f2, f3, 1.0F).tex((f4 + f6), (f4 + f6)).lightmap(i, i).endVertex();
      bufferbuilder.pos(x + 1.0D, y + offset, z + 1.0D).color(f1, f2, f3, 1.0F).tex((f4 + f6), (f5 + f6)).lightmap(i, i).endVertex();
      bufferbuilder.pos(x + 1.0D, y + offset, z).color(f1, f2, f3, 1.0F).tex((f5 + f6), (f5 + f6)).lightmap(i, i).endVertex();
      bufferbuilder.pos(x, y + offset, z).color(f1, f2, f3, 1.0F).tex((f5 + f6), (f4 + f6)).lightmap(i, i).endVertex();
    } 
    tessellator.draw();
...
/* Location:              C:\Users\BSV\AppData\Local\Temp\Rar$DRa6216.20396\Preview\Preview.jar!\shadersmod\client\ShadersRender.class
