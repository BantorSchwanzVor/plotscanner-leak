...
public class WorldClient extends World {
  ...
  public WorldClient(NetHandlerPlayClient netHandler, WorldSettings settings, int dimension, EnumDifficulty difficulty, Profiler profilerIn) {
    super((ISaveHandler)new SaveHandlerMP(), new WorldInfo(settings, "MpServer"), makeWorldProvider(dimension), profilerIn, true);
  ...
  public Entity getEntityByID(int id) {
    return (id == this.mc.player.getEntityId()) ? (Entity)this.mc.player : super.getEntityByID(id);
  }
  ...
  protected void playMoodSoundAndCheckLight(int p_147467_1_, int p_147467_2_, Chunk chunkIn) {
    ...
    if (this.ambienceTicks == 0) {
      ...
      j += p_147467_1_;
      k += p_147467_2_;
      double d0 = this.mc.player.getDistanceSq(j + 0.5D, l + 0.5D, k + 0.5D);
      if (d0 < 4.0D)
        return; 
      if (d0 > 255.0D)
        return; 
      if (iblockstate.getMaterial() == Material.AIR && getLight(blockpos) <= this.rand.nextInt(8) && getLightFor(EnumSkyBlock.SKY, blockpos) <= 0) {
        playSound(j + 0.5D, l + 0.5D, k + 0.5D, SoundEvents.AMBIENT_CAVE, SoundCategory.AMBIENT, 0.7F, 0.8F + this.rand.nextFloat() * 0.2F, false);
  ...
  public void showBarrierParticles(int p_184153_1_, int p_184153_2_, int p_184153_3_, int p_184153_4_, Random random, boolean p_184153_6_, BlockPos.MutableBlockPos pos) {
    ...
    pos.setPos(i, j, k);
    IBlockState iblockstate = getBlockState((BlockPos)pos);
    iblockstate.getBlock().randomDisplayTick(iblockstate, this, (BlockPos)pos, random);
    if (p_184153_6_ && iblockstate.getBlock() == Blocks.BARRIER)
      spawnParticle(EnumParticleTypes.BARRIER, (i + 0.5F), (j + 0.5F), (k + 0.5F), 0.0D, 0.0D, 0.0D, new int[0]); 
  }
  ...
  public void removeAllEntities() {
    ...
    for (int i1 = 0; i1 < this.unloadedEntityList.size(); i1++)
      onEntityRemoved(this.unloadedEntityList.get(i1)); 
  ...
  public void playSound(BlockPos pos, SoundEvent soundIn, SoundCategory category, float volume, float pitch, boolean distanceDelay) {
    playSound(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, soundIn, category, volume, pitch, distanceDelay);
  }
  ...
  public void makeFireworks(double x, double y, double z, double motionX, double motionY, double motionZ, @Nullable NBTTagCompound compund) {
    this.mc.effectRenderer.addEffect((Particle)new ParticleFirework.Starter(this, x, y, z, motionX, motionY, motionZ, this.mc.effectRenderer, compund));
  }
