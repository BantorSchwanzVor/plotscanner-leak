...
public class GuiContainerCreative extends InventoryEffectRenderer {
  ...
  protected void handleMouseClick(@Nullable Slot slotIn, int slotId, int mouseButton, ClickType type) {
    ...
    if (slotIn == null && selectedTabIndex != CreativeTabs.INVENTORY.getTabIndex() && type != ClickType.QUICK_CRAFT) {
      ...
      if (slotIn != null && !slotIn.canTakeStack((EntityPlayer)this.mc.player))
        return; 
      if (slotIn == this.destroyItemSlot && flag) {
        ...
        if (slotIn == this.destroyItemSlot) {
          ...
          this.mc.player.inventoryContainer.slotClick((slotIn == null) ? slotId : ((CreativeSlot)slotIn).slot.slotNumber, mouseButton, type, (EntityPlayer)this.mc.player);
        ...
        this.inventorySlots.slotClick((slotIn == null) ? slotId : slotIn.slotNumber, mouseButton, type, (EntityPlayer)this.mc.player);
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    if (mouseButton == 0) {
      ...
      for (k = (arrayOfCreativeTabs = CreativeTabs.CREATIVE_TAB_ARRAY).length, b = 0; b < k; ) {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    int m;
    CreativeTabs[] arrayOfCreativeTabs;
    for (m = (arrayOfCreativeTabs = CreativeTabs.CREATIVE_TAB_ARRAY).length, b = 0; b < m; ) {
      CreativeTabs creativetabs = arrayOfCreativeTabs[b];
      if (renderCreativeInventoryHoveringText(creativetabs, mouseX, mouseY))
        break; 
      b++;
    } 
    if (this.destroyItemSlot != null && selectedTabIndex == CreativeTabs.INVENTORY.getTabIndex() && isPointInRegion(this.destroyItemSlot.xDisplayPosition, this.destroyItemSlot.yDisplayPosition, 16, 16, mouseX, mouseY))
  ...
  protected void renderToolTip(ItemStack stack, int x, int y) {
    if (selectedTabIndex == CreativeTabs.SEARCH.getTabIndex()) {
      List<String> list = stack.getTooltip((EntityPlayer)this.mc.player, this.mc.gameSettings.advancedItemTooltips ? (ITooltipFlag)ITooltipFlag.TooltipFlags.ADVANCED : (ITooltipFlag)ITooltipFlag.TooltipFlags.NORMAL);
      ...
      if (creativetabs == null && stack.getItem() == Items.ENCHANTED_BOOK) {
        ...
        if (map.size() == 1) {
          ...
          int j;
          CreativeTabs[] arrayOfCreativeTabs;
          for (j = (arrayOfCreativeTabs = CreativeTabs.CREATIVE_TAB_ARRAY).length, b = 0; b < j; ) {
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    if (creativetabs == CreativeTabs.INVENTORY)
      GuiInventory.drawEntityOnScreen(this.guiLeft + 88, this.guiTop + 45, 20, (this.guiLeft + 88 - mouseX), (this.guiTop + 45 - 30 - mouseY), (EntityLivingBase)this.mc.player); 
  }
  ...
  public static class ContainerCreative extends Container {
    ...
    public ContainerCreative(EntityPlayer player) {
      ...
      for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 9; j++)
          addSlotToContainer(new GuiContainerCreative.LockedSlot((IInventory)GuiContainerCreative.basicInventory, i * 9 + j, 9 + j * 18, 18 + i * 18)); 
      } 
      for (int k = 0; k < 9; k++)
