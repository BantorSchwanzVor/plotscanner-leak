...
public class GuiEnchantment extends GuiContainer {
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      if (l >= 0 && i1 >= 0 && l < 108 && i1 < 19 && this.container.enchantItem((EntityPlayer)this.mc.player, k))
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    for (int l = 0; l < 3; l++) {
      ...
      if (k1 == 0) {
        ...
        int m = k1;
        int l1 = 86 - this.fontRendererObj.getStringWidth(m);
      ...
      if (isPointInRegion(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0 && l >= 0 && enchantment != null) {
        ...
        if (!flag) {
          ...
          if (this.mc.player.experienceLevel < k) {
            list.add(TextFormatting.RED + I18n.format("container.enchant.level.requirement", new Object[] { Integer.valueOf(this.container.enchantLevels[j]) }));
          } else {
            String s;
            if (i1 == 1) {
