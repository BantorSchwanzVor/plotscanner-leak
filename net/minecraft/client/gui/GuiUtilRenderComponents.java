  ...
  public static List<ITextComponent> splitText(ITextComponent textComponent, int maxTextLenght, FontRenderer fontRendererIn, boolean p_178908_3_, boolean forceTextColor) {
    ...
    for (int j = 0; j < list1.size(); j++) {
      ...
      if (s.contains("\n")) {
        ...
        s = s.substring(0, k + 1);
        TextComponentString textComponentString1 = new TextComponentString(s1);
        textComponentString1.setStyle(itextcomponent1.getStyle().createShallowCopy());
        list1.add(j + 1, textComponentString1);
        flag = true;
      } 
      String s4 = removeTextColorsIfConfigured(String.valueOf(itextcomponent1.getStyle().getFormattingCode()) + s, forceTextColor);
      ...
      if (i + i1 <= maxTextLenght) {
        i += i1;
        textComponentString.appendSibling((ITextComponent)textcomponentstring);
      } else {
        flag = true;
      } 
      if (flag) {
