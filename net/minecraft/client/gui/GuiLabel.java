...
public class GuiLabel extends Gui {
    ...
    if (this.visible) {
      ...
      for (int k = 0; k < this.labels.size(); k++) {
        if (this.centered) {
          ...
          drawString(this.fontRenderer, this.labels.get(k), this.x, j + k * 10, this.textColor);
        } 
      } 
    } 
  }
  ...
  protected void drawLabelBackground(Minecraft mcIn, int p_146160_2_, int p_146160_3_) {
    if (this.labelBgEnabled) {
      ...
      drawHorizontalLine(k, k + i, l, this.ulColor);
      drawHorizontalLine(k, k + i, l + j, this.brColor);
