...
public class RenderChunk {
      ...
      if (!chunkcacheof.isEmpty()) {
        ...
        for (Object blockposm0 : BlockPosM.getAllInBoxMutable((BlockPos)mutableBlockPos, blockpos1)) {
          ...
          for (int k = 0; k < ablockrenderlayer.length; k++) {
            BlockRenderLayer blockrenderlayer = ablockrenderlayer[k];
            if (flag) {
            ...
            int m = blockrenderlayer.ordinal();
            if (block.getDefaultState().getRenderType() != EnumBlockRenderType.INVISIBLE) {
              BufferBuilder bufferbuilder = generator.getRegionRenderCacheBuilder().getWorldRendererByLayerId(m);
              ...
              aboolean[m] = aboolean[m] | blockrendererdispatcher.renderBlock(iblockstate, (BlockPos)blockposm, (IBlockAccess)chunkcacheof, bufferbuilder);
        ...
        for (j = (arrayOfBlockRenderLayer = ENUM_WORLD_BLOCK_LAYERS).length, b = 0; b < j; ) {
          BlockRenderLayer blockrenderlayer1 = arrayOfBlockRenderLayer[b];
  ...
  public Chunk getChunk(World p_getChunk_1_) {
    if (this.chunk != null && this.chunk.isLoaded())
      return this.chunk; 
    this.chunk = p_getChunk_1_.getChunkFromBlockCoords(getPosition());
    return this.chunk;
  }
