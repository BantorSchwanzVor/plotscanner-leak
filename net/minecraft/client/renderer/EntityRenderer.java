...
public class EntityRenderer implements IResourceManagerReloadListener {
  ...
  private void orientCamera(float partialTicks) {
    ...
    if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
      if (!this.mc.gameSettings.debugCamEnable) {
        ...
        IBlockState iblockstate1 = ActiveRenderInfo.getBlockStateAtEntityViewpoint((World)this.mc.world, entity, partialTicks);
  ...
  private void updateLightmap(float partialTicks) {
    if (this.lightmapUpdateNeeded) {
      ...
      if (worldClient != null) {
        if (Config.isCustomColors() && CustomColors.updateLightmap((World)worldClient, this.torchFlickerX, this.lightmapColors, this.mc.player.isPotionActive(MobEffects.NIGHT_VISION))) {
        ...
        for (int i = 0; i < 256; i++) {
          ...
          if (((World)worldClient).provider.getDimensionType().getId() == 1) {
            f8 = 0.22F + f3 * 0.75F;
            f9 = 0.28F + f6 * 0.75F;
            f10 = 0.25F + f7 * 0.75F;
          } 
          if (Reflector.ForgeWorldProvider_getLightmapColors.exists()) {
            ...
            Reflector.call(((World)worldClient).provider, Reflector.ForgeWorldProvider_getLightmapColors, new Object[] { Float.valueOf(partialTicks), Float.valueOf(f), Float.valueOf(f2), Float.valueOf(f3), afloat });
            f8 = afloat[0];
            f9 = afloat[1];
            f10 = afloat[2];
          } 
          ...
          if (this.mc.player.isPotionActive(MobEffects.NIGHT_VISION)) {
            float f15 = getNightVisionBrightness((EntityLivingBase)this.mc.player, partialTicks);
            float f12 = 1.0F / f8;
            if (f12 > 1.0F / f9)
              f12 = 1.0F / f9; 
            if (f12 > 1.0F / f10)
      ...
      if (this.mc.currentScreen != null) {
        ...
        try {
          if (Reflector.ForgeHooksClient_drawScreen.exists()) {
            Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, new Object[] { this.mc.currentScreen, Integer.valueOf(k1), Integer.valueOf(l1), Float.valueOf(this.mc.func_193989_ak()) });
          } else {
    ...
    if (flag && !((EntityPlayer)entity).capabilities.allowEdit) {
      ...
      if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
        ...
        if (this.mc.playerController.getCurrentGameType() == GameType.SPECTATOR) {
          flag = (ReflectorForge.blockHasTileEntity(iblockstate) && this.mc.world.getTileEntity(blockpos) instanceof net.minecraft.inventory.IInventory);
        } else {
  ...
  private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
    ...
    ActiveRenderInfo.updateRenderInfo((EntityPlayer)this.mc.player, (this.mc.gameSettings.thirdPersonView == 2));
    ...
    checkLoadVisibleChunks(entity, partialTicks, (ICamera)frustum, this.mc.player.isSpectator());
    if (flag) {
      ShadersRender.setupTerrain(renderglobal, entity, partialTicks, (ICamera)frustum, this.frameCount++, this.mc.player.isSpectator());
    } else {
      renderglobal.setupTerrain(entity, partialTicks, (ICamera)frustum, this.frameCount++, this.mc.player.isSpectator());
    } 
    if (pass == 0 || pass == 2) {
  ...
  private void addRainParticles() {
    ...
    if (f != 0.0F && Config.isRainSplash()) {
      ...
      for (int l = 0; l < k; l++) {
        BlockPos blockpos1 = worldClient.getPrecipitationHeight(blockpos.add(this.random.nextInt(10) - this.random.nextInt(10), 0, this.random.nextInt(10) - this.random.nextInt(10)));
        Biome biome = worldClient.getBiome(blockpos1);
        ...
        if (blockpos1.getY() <= blockpos.getY() + 10 && blockpos1.getY() >= blockpos.getY() - 10 && biome.canRain() && biome.getFloatTemperature(blockpos1) >= 0.15F) {
          ...
          AxisAlignedBB axisalignedbb = iblockstate.getBoundingBox((IBlockAccess)worldClient, blockpos2);
      ...
      if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
        ...
        if (d1 > (blockpos.getY() + 1) && worldClient.getPrecipitationHeight(blockpos).getY() > MathHelper.floor(blockpos.getY())) {
  ...
  private void updateFogColor(float partialTicks) {
    ...
    vec3d = CustomColors.getWorldSkyColor(vec3d, (World)worldClient, this.mc.getRenderViewEntity(), partialTicks);
    float f1 = (float)vec3d.xCoord;
    float f2 = (float)vec3d.yCoord;
    float f3 = (float)vec3d.zCoord;
    Vec3d vec3d1 = worldClient.getFogColor(partialTicks);
    vec3d1 = CustomColors.getWorldFogColor(vec3d1, (World)worldClient, this.mc.getRenderViewEntity(), partialTicks);
    this.fogColorRed = (float)vec3d1.xCoord;
    this.fogColorGreen = (float)vec3d1.yCoord;
    this.fogColorBlue = (float)vec3d1.zCoord;
    if (this.mc.gameSettings.renderDistanceChunks >= 4) {
      double d0 = (MathHelper.sin(worldClient.getCelestialAngleRadians(partialTicks)) > 0.0F) ? -1.0D : 1.0D;
    ...
    IBlockState iblockstate1 = ActiveRenderInfo.getBlockStateAtEntityViewpoint((World)this.mc.world, entity, partialTicks);
    if (this.cloudFog) {
      Vec3d vec3d4 = worldClient.getCloudColour(partialTicks);
      ...
      IBlockState iblockstate = this.mc.world.getBlockState(blockpos);
      Vec3d vec3d3 = (Vec3d)Reflector.call(iblockstate.getBlock(), Reflector.ForgeBlock_getFogColor, new Object[] { this.mc.world, blockpos, iblockstate, entity, new Vec3d(this.fogColorRed, this.fogColorGreen, this.fogColorBlue), Float.valueOf(partialTicks) });
      ...
      Vec3d vec3d7 = CustomColors.getUnderwaterColor((IBlockAccess)this.mc.world, (this.mc.getRenderViewEntity()).posX, (this.mc.getRenderViewEntity()).posY + 1.0D, (this.mc.getRenderViewEntity()).posZ);
      if (vec3d7 != null) {
      ...
      Vec3d vec3d6 = CustomColors.getUnderlavaColor((IBlockAccess)this.mc.world, (this.mc.getRenderViewEntity()).posX, (this.mc.getRenderViewEntity()).posY + 1.0D, (this.mc.getRenderViewEntity()).posZ);
      if (vec3d6 != null) {
    ...
    if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
      Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor, new Object[] { this, entity, iblockstate1, Float.valueOf(partialTicks), Float.valueOf(this.fogColorRed), Float.valueOf(this.fogColorGreen), Float.valueOf(this.fogColorBlue) });
      Reflector.postForgeBusEvent(object);
  ...
  private void frameInit() {
    ...
    if (worldClient != null) {
      ...
      if (Config.isNotify64BitJava()) {
        ...
        this.mc.ingameGUI.getChatGUI().printChatMessage((ITextComponent)textcomponentstring1);
      } 
    } 
  ...
  private void checkLoadVisibleChunks(Entity p_checkLoadVisibleChunks_1_, float p_checkLoadVisibleChunks_2_, ICamera p_checkLoadVisibleChunks_3_, boolean p_checkLoadVisibleChunks_4_) {
    ...
    if (Keyboard.isKeyDown(61) && Keyboard.isKeyDown(38)) {
      if (this.mc.gameSettings.field_194146_ao.getKeyCode() == 38) {
        if (this.mc.currentScreen instanceof net.minecraft.client.gui.advancements.GuiScreenAdvancements)
      ...
      this.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion((ITextComponent)textcomponentstring, i);
