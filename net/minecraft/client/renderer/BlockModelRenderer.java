...
public class BlockModelRenderer {
  ...
  public boolean renderModel(IBlockAccess blockAccessIn, IBakedModel modelIn, IBlockState blockStateIn, BlockPos blockPosIn, BufferBuilder buffer, boolean checkSides) {
    return renderModel(blockAccessIn, modelIn, blockStateIn, blockPosIn, buffer, checkSides, MathHelper.getPositionRandom((Vec3i)blockPosIn));
  }
  ...
  public boolean renderModelFlat(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
    ...
    for (i = (arrayOfEnumFacing = EnumFacing.VALUES).length, b = 0; b < i; ) {
      ...
      List<BakedQuad> list = modelIn.getQuads(stateIn, enumfacing, rand);
      if (!list.isEmpty() && (!checkSides || stateIn.shouldSideBeRendered(worldIn, posIn, enumfacing))) {
        int j = stateIn.getPackedLightmapCoords(worldIn, posIn.offset(enumfacing));
        list = BlockModelCustomizer.getRenderQuads(list, worldIn, stateIn, posIn, enumfacing, rand, renderenv);
        renderQuadsFlat(worldIn, stateIn, posIn, j, false, buffer, list, renderenv);
        flag = true;
      } 
      b++;
    } 
  ...
  private void renderQuadsSmooth(IBlockAccess p_renderQuadsSmooth_1_, IBlockState p_renderQuadsSmooth_2_, BlockPos p_renderQuadsSmooth_3_, BufferBuilder p_renderQuadsSmooth_4_, List<BakedQuad> p_renderQuadsSmooth_5_, RenderEnv p_renderQuadsSmooth_6_) {
    ...
    double d0 = p_renderQuadsSmooth_3_.getX() + vec3d.xCoord;
    double d1 = p_renderQuadsSmooth_3_.getY() + vec3d.yCoord;
    double d2 = p_renderQuadsSmooth_3_.getZ() + vec3d.zCoord;
    int i = 0;
  ...
  private void renderQuadsFlat(IBlockAccess p_renderQuadsFlat_1_, IBlockState p_renderQuadsFlat_2_, BlockPos p_renderQuadsFlat_3_, int p_renderQuadsFlat_4_, boolean p_renderQuadsFlat_5_, BufferBuilder p_renderQuadsFlat_6_, List<BakedQuad> p_renderQuadsFlat_7_, RenderEnv p_renderQuadsFlat_8_) {
    ...
    double d0 = p_renderQuadsFlat_3_.getX() + vec3d.xCoord;
    double d1 = p_renderQuadsFlat_3_.getY() + vec3d.yCoord;
    double d2 = p_renderQuadsFlat_3_.getZ() + vec3d.zCoord;
    int i = 0;
  ...
  public static class AmbientOcclusionFace {
    ...
    public void updateVertexBrightness(IBlockAccess worldIn, IBlockState state, BlockPos centerPos, EnumFacing direction, float[] faceShape, BitSet shapeState) {
      ...
      int i = state.getPackedLightmapCoords(worldIn, (BlockPos)blockpos$pooledmutableblockpos1);
      int j = state.getPackedLightmapCoords(worldIn, (BlockPos)blockpos$pooledmutableblockpos2);
      int k = state.getPackedLightmapCoords(worldIn, (BlockPos)blockpos$pooledmutableblockpos3);
      int l = state.getPackedLightmapCoords(worldIn, (BlockPos)blockpos$pooledmutableblockpos4);
      float f = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos1).getAmbientOcclusionLightValue();
      float f1 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos2).getAmbientOcclusionLightValue();
      float f2 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos3).getAmbientOcclusionLightValue();
      float f3 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos4).getAmbientOcclusionLightValue();
      ...
      boolean flag = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos1).move(direction)).isTranslucent();
      boolean flag1 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos2).move(direction)).isTranslucent();
      boolean flag2 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos3).move(direction)).isTranslucent();
      boolean flag3 = worldIn.getBlockState((BlockPos)blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos4).move(direction)).isTranslucent();
      if (!flag2 && !flag) {
        f25 = f;
        i1 = i;
      } else {
        BlockPos.PooledMutableBlockPos pooledMutableBlockPos = blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos1).move(blockmodelrenderer$enumneighborinfo.corners[2]);
        f25 = worldIn.getBlockState((BlockPos)pooledMutableBlockPos).getAmbientOcclusionLightValue();
        f25 = BlockModelRenderer.fixAoLightValue(f25);
        i1 = state.getPackedLightmapCoords(worldIn, (BlockPos)pooledMutableBlockPos);
      } 
      if (!flag3 && !flag) {
        f26 = f;
        j1 = i;
      } else {
        BlockPos.PooledMutableBlockPos pooledMutableBlockPos = blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos1).move(blockmodelrenderer$enumneighborinfo.corners[3]);
        f26 = worldIn.getBlockState((BlockPos)pooledMutableBlockPos).getAmbientOcclusionLightValue();
        f26 = BlockModelRenderer.fixAoLightValue(f26);
        j1 = state.getPackedLightmapCoords(worldIn, (BlockPos)pooledMutableBlockPos);
      } 
      if (!flag2 && !flag1) {
        f27 = f1;
        k1 = j;
      } else {
        BlockPos.PooledMutableBlockPos pooledMutableBlockPos = blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos2).move(blockmodelrenderer$enumneighborinfo.corners[2]);
        f27 = worldIn.getBlockState((BlockPos)pooledMutableBlockPos).getAmbientOcclusionLightValue();
        f27 = BlockModelRenderer.fixAoLightValue(f27);
        k1 = state.getPackedLightmapCoords(worldIn, (BlockPos)pooledMutableBlockPos);
      } 
      if (!flag3 && !flag1) {
        f28 = f1;
        l1 = j;
      } else {
        BlockPos.PooledMutableBlockPos pooledMutableBlockPos = blockpos$pooledmutableblockpos.setPos((Vec3i)blockpos$pooledmutableblockpos2).move(blockmodelrenderer$enumneighborinfo.corners[3]);
        f28 = worldIn.getBlockState((BlockPos)pooledMutableBlockPos).getAmbientOcclusionLightValue();
        f28 = BlockModelRenderer.fixAoLightValue(f28);
        l1 = state.getPackedLightmapCoords(worldIn, (BlockPos)pooledMutableBlockPos);
      } 
      ...
      if (shapeState.get(1) && blockmodelrenderer$enumneighborinfo.doNonCubicWeight) {
        ...
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert0] = getVertexBrightness(i2, j2, k2, l2, f9, f10, f11, f12);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert1] = getVertexBrightness(i2, j2, k2, l2, f13, f14, f15, f16);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert2] = getVertexBrightness(i2, j2, k2, l2, f17, f18, f19, f20);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert3] = getVertexBrightness(i2, j2, k2, l2, f21, f22, f23, f24);
      } else {
        float f5 = (f3 + f + f26 + f4) * 0.25F;
        float f6 = (f2 + f + f25 + f4) * 0.25F;
        float f7 = (f2 + f1 + f27 + f4) * 0.25F;
        float f8 = (f3 + f1 + f28 + f4) * 0.25F;
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert0] = getAoBrightness(l, i, j1, i3);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert1] = getAoBrightness(k, i, i1, i3);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert2] = getAoBrightness(k, j, k1, i3);
        this.vertexBrightness[blockmodelrenderer$vertextranslations.vert3] = getAoBrightness(l, j, l1, i3);
