  ...
  public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    ...
    if (i > 0) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        ((Entity)entityTippedArrow).rotationYaw = (float)(Math.atan2(f, f2) * 57.29577951308232D);
        ((Entity)entityTippedArrow).rotationPitch = (float)(Math.atan2(f1, f6) * 57.29577951308232D);
        ((Entity)entityTippedArrow).prevRotationYaw = ((Entity)entityTippedArrow).rotationYaw;
        ((Entity)entityTippedArrow).prevRotationPitch = ((Entity)entityTippedArrow).rotationPitch;
        double d0 = 0.0D;
        double d1 = 0.0D;
        double d2 = 0.0D;
        this.renderer.getRenderManager().doRenderEntity((Entity)entityTippedArrow, 0.0D, 0.0D, 0.0D, 0.0F, partialTicks, false);
