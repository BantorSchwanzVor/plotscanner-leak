  ...
  public void render(float p_190060_1_, long p_190060_2_) {
    ...
    for (BlockPos blockpos1 : BlockPos.getAllInBox(blockpos.add(-10, -10, -10), blockpos.add(10, 10, 10))) {
      ...
      if (iblockstate.getBlock() == Blocks.WATER || iblockstate.getBlock() == Blocks.FLOWING_WATER) {
        double d0 = BlockLiquid.func_190972_g(iblockstate, (IBlockAccess)world, blockpos1);
    ...
    for (BlockPos blockpos2 : BlockPos.getAllInBox(blockpos.add(-10, -10, -10), blockpos.add(10, 10, 10))) {
      ...
      if (iblockstate1.getBlock() == Blocks.WATER || iblockstate1.getBlock() == Blocks.FLOWING_WATER) {
        Integer integer = (Integer)iblockstate1.getValue((IProperty)BlockLiquid.LEVEL);
        double d1 = (integer.intValue() > 7) ? 0.9D : (1.0D - 0.11D * integer.intValue());
        String s = (iblockstate1.getBlock() == Blocks.FLOWING_WATER) ? "f" : "s";
        DebugRenderer.renderDebugText(String.valueOf(s) + " " + integer, blockpos2.getX() + 0.5D, blockpos2.getY() + d1, blockpos2.getZ() + 0.5D, p_190060_1_, -16777216);
      } 
    } 
