...
public class ConditionPropertyValue implements ICondition {
  ...
  public Predicate<IBlockState> getPredicate(BlockStateContainer blockState) {
    ...
    if (iproperty == null)
      throw new RuntimeException(String.valueOf(toString()) + ": Definition: " + blockState + " has no property: " + this.key); 
    String s = this.value;
    ...
    if (list.isEmpty())
      throw new RuntimeException(String.valueOf(toString()) + ": has an empty value: " + this.value); 
    if (list.size() == 1) {
  ...
  private Predicate<IBlockState> makePredicate(final IProperty<?> property, String valueIn) {
    ...
    if (!optional.isPresent())
      throw new RuntimeException(String.valueOf(toString()) + ": has an unknown value: " + this.value); 
