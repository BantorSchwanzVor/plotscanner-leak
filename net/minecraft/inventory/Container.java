...
public abstract class Container {
  ...
  public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
    ...
    if (clickTypeIn == ClickType.QUICK_CRAFT) {
      ...
      if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
        ...
        if (slot7 != null && canAddItemToSlot(slot7, itemstack12, true) && slot7.isItemValid(itemstack12) && (this.dragMode == 2 || itemstack12.func_190916_E() > this.dragSlots.size()) && canDragIntoSlot(slot7))
        ...
        if (!this.dragSlots.isEmpty()) {
          ...
          for (Slot slot8 : this.dragSlots) {
            ...
            if (slot8 != null && canAddItemToSlot(slot8, itemstack13, true) && slot8.isItemValid(itemstack13) && (this.dragMode == 2 || itemstack13.func_190916_E() >= this.dragSlots.size()) && canDragIntoSlot(slot8)) {
      ...
      if (slotId == -999) {
        ...
        if (slot5 == null || !slot5.canTakeStack(player))
          return ItemStack.field_190927_a; 
        for (ItemStack itemstack7 = transferStackInSlot(player, slotId); !itemstack7.func_190926_b() && ItemStack.areItemsEqual(slot5.getStack(), itemstack7); itemstack7 = transferStackInSlot(player, slotId))
  ...
  public static void computeStackSize(Set<Slot> dragSlotsIn, int dragModeIn, ItemStack stack, int slotStackSize) {
    switch (dragModeIn) {
      case 0:
        stack.func_190920_e(MathHelper.floor(stack.func_190916_E() / dragSlotsIn.size()));
        break;
      case 1:
  ...
  public static int calcRedstoneFromInventory(@Nullable IInventory inv) {
    ...
    for (int j = 0; j < inv.getSizeInventory(); j++) {
      ...
      if (!itemstack.func_190926_b()) {
        f += itemstack.func_190916_E() / Math.min(inv.getInventoryStackLimit(), itemstack.getMaxStackSize());
        i++;
      } 
    } 
  ...
  protected void func_192389_a(World p_192389_1_, EntityPlayer p_192389_2_, InventoryCrafting p_192389_3_, InventoryCraftResult p_192389_4_) {
    if (!p_192389_1_.isRemote) {
      ...
      entityplayermp.connection.sendPacket((Packet)new SPacketSetSlot(this.windowId, 0, itemstack));
    } 
  }
}
