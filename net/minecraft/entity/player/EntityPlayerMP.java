...
public class EntityPlayerMP extends EntityPlayer implements IContainerListener {
  ...
  public EntityPlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, PlayerInteractionManager interactionManagerIn) {
    ...
    while (!worldIn.getCollisionBoxes((Entity)this, getEntityBoundingBox()).isEmpty() && this.posY < 255.0D)
  ...
  public void writeEntityToNBT(NBTTagCompound compound) {
    ...
    if (this.field_193110_cw != null) {
      ...
      compound.setTag("enteredNetherPosition", (NBTBase)nbttagcompound);
    } 
    ...
    if (entity != null && entity1 != this && entity1.getRecursivePassengersByType(EntityPlayerMP.class).size() == 1) {
      ...
      nbttagcompound1.setTag("Entity", (NBTBase)nbttagcompound2);
      compound.setTag("RootVehicle", (NBTBase)nbttagcompound1);
    } 
    compound.setTag("recipeBook", (NBTBase)this.field_192041_cq.func_192824_e());
  }
  ...
  public void sendEnterCombat() {
    ...
    this.connection.sendPacket((Packet)new SPacketCombatEvent(getCombatTracker(), SPacketCombatEvent.Event.ENTER_COMBAT));
  }
  
  public void sendEndCombat() {
    super.sendEndCombat();
    this.connection.sendPacket((Packet)new SPacketCombatEvent(getCombatTracker(), SPacketCombatEvent.Event.END_COMBAT));
  }
  ...
  protected CooldownTracker createCooldownTracker() {
    return (CooldownTracker)new CooldownTrackerServer(this);
  }
  ...
  public void onUpdate() {
    ...
    while (!this.entityRemoveQueue.isEmpty()) {
      int i = Math.min(this.entityRemoveQueue.size(), 2147483647);
      int[] aint = new int[i];
      ...
      this.connection.sendPacket((Packet)new SPacketDestroyEntities(aint));
    } 
  ...
  public void onUpdateEntity() {
    try {
      ...
      if (getHealth() != this.lastHealth || this.lastFoodLevel != this.foodStats.getFoodLevel() || ((this.foodStats.getSaturationLevel() == 0.0F)) != this.wasHungry) {
        this.connection.sendPacket((Packet)new SPacketUpdateHealth(getHealth(), this.foodStats.getFoodLevel(), this.foodStats.getSaturationLevel()));
        this.lastHealth = getHealth();
        this.lastFoodLevel = this.foodStats.getFoodLevel();
        this.wasHungry = (this.foodStats.getSaturationLevel() == 0.0F);
      } 
      if (getHealth() + getAbsorptionAmount() != this.lastHealthScore) {
        this.lastHealthScore = getHealth() + getAbsorptionAmount();
        updateScorePoints(IScoreCriteria.HEALTH, MathHelper.ceil(this.lastHealthScore));
      } 
      if (this.foodStats.getFoodLevel() != this.lastFoodScore) {
        this.lastFoodScore = this.foodStats.getFoodLevel();
        updateScorePoints(IScoreCriteria.FOOD, MathHelper.ceil(this.lastFoodScore));
      } 
      if (getAir() != this.lastAirScore) {
        this.lastAirScore = getAir();
        updateScorePoints(IScoreCriteria.AIR, MathHelper.ceil(this.lastAirScore));
      } 
      if (getTotalArmorValue() != this.lastArmorScore) {
        this.lastArmorScore = getTotalArmorValue();
        updateScorePoints(IScoreCriteria.ARMOR, MathHelper.ceil(this.lastArmorScore));
      } 
      if (this.experienceTotal != this.lastExperienceScore) {
        this.lastExperienceScore = this.experienceTotal;
        updateScorePoints(IScoreCriteria.XP, MathHelper.ceil(this.lastExperienceScore));
      } 
      if (this.experienceLevel != this.lastLevelScore) {
        this.lastLevelScore = this.experienceLevel;
        updateScorePoints(IScoreCriteria.LEVEL, MathHelper.ceil(this.lastLevelScore));
      } 
      if (this.experienceTotal != this.lastExperience) {
        this.lastExperience = this.experienceTotal;
        this.connection.sendPacket((Packet)new SPacketSetExperience(this.experience, this.experienceTotal, this.experienceLevel));
      } 
  ...
  public void onDeath(DamageSource cause) {
    ...
    this.connection.sendPacket((Packet)new SPacketCombatEvent(getCombatTracker(), SPacketCombatEvent.Event.ENTITY_DIED, flag));
    if (flag) {
      Team team = getTeam();
      if (team != null && team.getDeathMessageVisibility() != Team.EnumVisible.ALWAYS) {
        if (team.getDeathMessageVisibility() == Team.EnumVisible.HIDE_FOR_OTHER_TEAMS) {
          this.mcServer.getPlayerList().sendMessageToAllTeamMembers(this, getCombatTracker().getDeathMessage());
        } else if (team.getDeathMessageVisibility() == Team.EnumVisible.HIDE_FOR_OWN_TEAM) {
          this.mcServer.getPlayerList().sendMessageToTeamOrAllPlayers(this, getCombatTracker().getDeathMessage());
        } 
      } else {
        this.mcServer.getPlayerList().sendChatMsg(getCombatTracker().getDeathMessage());
      } 
    } 
    func_192030_dh();
    ...
    if (entitylivingbase != null) {
      EntityList.EntityEggInfo entitylist$entityegginfo = (EntityList.EntityEggInfo)EntityList.ENTITY_EGGS.get(EntityList.func_191301_a((Entity)entitylivingbase));
      if (entitylist$entityegginfo != null)
        addStat(entitylist$entityegginfo.entityKilledByStat); 
      entitylivingbase.func_191956_a((Entity)this, this.scoreValue, cause);
    } 
  ...
  public Entity changeDimension(int dimensionIn) {
    ...
    if (this.dimension == 1 && dimensionIn == 1) {
      ...
      if (!this.playerConqueredTheEnd) {
        ...
        this.connection.sendPacket((Packet)new SPacketChangeGameState(4, this.field_192040_cp ? 0.0F : 1.0F));
    ...
    this.connection.sendPacket((Packet)new SPacketEffect(1032, BlockPos.ORIGIN, 0, false));
    this.lastExperience = -1;
  ...
  private void sendTileEntityUpdate(TileEntity p_147097_1_) {
    if (p_147097_1_ != null) {
      ...
      if (spacketupdatetileentity != null)
        this.connection.sendPacket((Packet)spacketupdatetileentity); 
    } 
  }
  ...
  public EntityPlayer.SleepResult trySleep(BlockPos bedLocation) {
    ...
    if (entityplayer$sleepresult == EntityPlayer.SleepResult.OK) {
      ...
      SPacketUseBed sPacketUseBed = new SPacketUseBed(this, bedLocation);
      getServerWorld().getEntityTracker().sendToAllTrackingEntity((Entity)this, (Packet)sPacketUseBed);
      this.connection.setPlayerLocation(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
      this.connection.sendPacket((Packet)sPacketUseBed);
  ...
  public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
    if (isPlayerSleeping())
      getServerWorld().getEntityTracker().sendToTrackingAndSelf((Entity)this, (Packet)new SPacketAnimation((Entity)this, 2)); 
  ...
  public void openEditSign(TileEntitySign signTile) {
    ...
    this.connection.sendPacket((Packet)new SPacketSignEditorOpen(signTile.getPos()));
  }
  ...
  public void displayGui(IInteractionObject guiOwner) {
    if (guiOwner instanceof ILootContainer && ((ILootContainer)guiOwner).getLootTable() != null && isSpectator()) {
      addChatComponentMessage((new TextComponentTranslation("container.spectatorCantOpen", new Object[0])).setStyle((new Style()).setColor(TextFormatting.RED)), true);
    } else {
      getNextWindowId();
      this.connection.sendPacket((Packet)new SPacketOpenWindow(this.currentWindowId, guiOwner.getGuiID(), guiOwner.getDisplayName()));
    ...
    if (chestInventory instanceof ILootContainer && ((ILootContainer)chestInventory).getLootTable() != null && isSpectator()) {
      ...
      if (chestInventory instanceof ILockableContainer) {
        ...
        if (ilockablecontainer.isLocked() && !canOpen(ilockablecontainer.getLockCode()) && !isSpectator()) {
          this.connection.sendPacket((Packet)new SPacketChat((ITextComponent)new TextComponentTranslation("container.isLocked", new Object[] { chestInventory.getDisplayName() }), ChatType.GAME_INFO));
          this.connection.sendPacket((Packet)new SPacketSoundEffect(SoundEvents.BLOCK_CHEST_LOCKED, SoundCategory.BLOCKS, this.posX, this.posY, this.posZ, 1.0F, 1.0F));
          return;
        } 
      } 
      getNextWindowId();
      if (chestInventory instanceof IInteractionObject) {
        this.connection.sendPacket((Packet)new SPacketOpenWindow(this.currentWindowId, ((IInteractionObject)chestInventory).getGuiID(), chestInventory.getDisplayName(), chestInventory.getSizeInventory()));
        this.openContainer = ((IInteractionObject)chestInventory).createContainer(this.inventory, this);
      } else {
        this.connection.sendPacket((Packet)new SPacketOpenWindow(this.currentWindowId, "minecraft:container", chestInventory.getDisplayName(), chestInventory.getSizeInventory()));
  ...
  public void displayVillagerTradeGui(IMerchant villager) {
    getNextWindowId();
    this.openContainer = (Container)new ContainerMerchant(this.inventory, villager, this.world);
    this.openContainer.windowId = this.currentWindowId;
    this.openContainer.addListener(this);
    ...
    this.connection.sendPacket((Packet)new SPacketOpenWindow(this.currentWindowId, "minecraft:villager", itextcomponent, inventoryMerchant.getSizeInventory()));
    MerchantRecipeList merchantrecipelist = villager.getRecipes(this);
    if (merchantrecipelist != null) {
      PacketBuffer packetbuffer = new PacketBuffer(Unpooled.buffer());
      packetbuffer.writeInt(this.currentWindowId);
      merchantrecipelist.writeToBuf(packetbuffer);
      this.connection.sendPacket((Packet)new SPacketCustomPayload("MC|TrList", packetbuffer));
    } 
  }
  ...
  public void openGuiHorseInventory(AbstractHorse horse, IInventory inventoryIn) {
    if (this.openContainer != this.inventoryContainer)
      closeScreen(); 
    getNextWindowId();
    this.connection.sendPacket((Packet)new SPacketOpenWindow(this.currentWindowId, "EntityHorse", inventoryIn.getDisplayName(), inventoryIn.getSizeInventory(), horse.getEntityId()));
    this.openContainer = (Container)new ContainerHorseInventory(this.inventory, inventoryIn, horse, this);
    this.openContainer.windowId = this.currentWindowId;
    this.openContainer.addListener(this);
  }
  ...
  public void openBook(ItemStack stack, EnumHand hand) {
    ...
    if (item == Items.WRITTEN_BOOK) {
      PacketBuffer packetbuffer = new PacketBuffer(Unpooled.buffer());
      packetbuffer.writeEnumValue((Enum)hand);
      this.connection.sendPacket((Packet)new SPacketCustomPayload("MC|BOpen", packetbuffer));
    } 
  }
  
  public void displayGuiCommandBlock(TileEntityCommandBlock commandBlock) {
    commandBlock.setSendToClient(true);
    sendTileEntityUpdate((TileEntity)commandBlock);
  }
  ...
  public void sendSlotContents(Container containerToSend, int slotInd, ItemStack stack) {
    if (!(containerToSend.getSlot(slotInd) instanceof net.minecraft.inventory.SlotCrafting)) {
      ...
      if (!this.isChangingQuantityOnly)
        this.connection.sendPacket((Packet)new SPacketSetSlot(containerToSend.windowId, slotInd, stack)); 
    } 
  }
  ...
  public void updateCraftingInventory(Container containerToSend, NonNullList<ItemStack> itemsList) {
    this.connection.sendPacket((Packet)new SPacketWindowItems(containerToSend.windowId, itemsList));
    this.connection.sendPacket((Packet)new SPacketSetSlot(-1, -1, this.inventory.getItemStack()));
  }
  
  public void sendProgressBarUpdate(Container containerIn, int varToUpdate, int newValue) {
    this.connection.sendPacket((Packet)new SPacketWindowProperty(containerIn.windowId, varToUpdate, newValue));
  }
  
  public void sendAllWindowProperties(Container containerIn, IInventory inventory) {
    for (int i = 0; i < inventory.getFieldCount(); i++)
      this.connection.sendPacket((Packet)new SPacketWindowProperty(containerIn.windowId, i, inventory.getField(i))); 
  }
  
  public void closeScreen() {
    this.connection.sendPacket((Packet)new SPacketCloseWindow(this.openContainer.windowId));
    closeContainer();
  }
  ...
  public void func_193102_a(ResourceLocation[] p_193102_1_) {
    ...
    for (i = (arrayOfResourceLocation = p_193102_1_).length, b = 0; b < i; ) {
      ResourceLocation resourcelocation = arrayOfResourceLocation[b];
  ...
  public void addChatComponentMessage(ITextComponent chatComponent, boolean p_146105_2_) {
    this.connection.sendPacket((Packet)new SPacketChat(chatComponent, p_146105_2_ ? ChatType.GAME_INFO : ChatType.CHAT));
  }
  
  protected void onItemUseFinish() {
    if (!this.activeItemStack.func_190926_b() && isHandActive()) {
      this.connection.sendPacket((Packet)new SPacketEntityStatus((Entity)this, (byte)9));
  ...
  public void func_193104_a(EntityPlayerMP p_193104_1_, boolean p_193104_2_) {
    ...
    getDataManager().set(PLAYER_MODEL_FLAG, p_193104_1_.getDataManager().get(PLAYER_MODEL_FLAG));
    this.lastExperience = -1;
    this.lastHealth = -1.0F;
    this.lastFoodLevel = -1;
  ...
  protected void onNewPotionEffect(PotionEffect id) {
    ...
    this.connection.sendPacket((Packet)new SPacketEntityEffect(getEntityId(), id));
  ...
  protected void onFinishedPotionEffect(PotionEffect effect) {
    ...
    this.connection.sendPacket((Packet)new SPacketRemoveEntityEffect(getEntityId(), effect.getPotion()));
  ...
  public void onCriticalHit(Entity entityHit) {
    getServerWorld().getEntityTracker().sendToTrackingAndSelf((Entity)this, (Packet)new SPacketAnimation(entityHit, 4));
  }
  
  public void onEnchantmentCritical(Entity entityHit) {
    getServerWorld().getEntityTracker().sendToTrackingAndSelf((Entity)this, (Packet)new SPacketAnimation(entityHit, 5));
  }
  
  public void sendPlayerAbilities() {
    if (this.connection != null) {
      this.connection.sendPacket((Packet)new SPacketPlayerAbilities(this.capabilities));
  ...
  public void setGameType(GameType gameType) {
    ...
    this.connection.sendPacket((Packet)new SPacketChangeGameState(3, gameType.getID()));
  ...
  public void addChatMessage(ITextComponent component) {
    this.connection.sendPacket((Packet)new SPacketChat(component));
  }
  ...
  public boolean canCommandSenderUseCommand(int permLevel, String commandName) {
    ...
    if (!"tell".equals(commandName) && !"help".equals(commandName) && !"me".equals(commandName) && !"trigger".equals(commandName)) {
      if (this.mcServer.getPlayerList().canSendCommands(getGameProfile())) {
        UserListOpsEntry userlistopsentry = (UserListOpsEntry)this.mcServer.getPlayerList().getOppedPlayers().getEntry(getGameProfile());
  ...
  public void loadResourcePack(String url, String hash) {
    this.connection.sendPacket((Packet)new SPacketResourcePackSend(url, hash));
  }
  ...
  public void removeEntity(Entity entityIn) {
    if (entityIn instanceof EntityPlayer) {
      this.connection.sendPacket((Packet)new SPacketDestroyEntities(new int[] { entityIn.getEntityId() }));
    } else {
  ...
  public void setSpectatingEntity(Entity entityToSpectate) {
    ...
    if (entity != this.spectatingEntity) {
      this.connection.sendPacket((Packet)new SPacketCamera(this.spectatingEntity));
