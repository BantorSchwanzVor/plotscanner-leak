...
public class InventoryPlayer implements IInventory {
  ...
  public InventoryPlayer(EntityPlayer playerIn) {
    this.allInventories = Arrays.asList((NonNullList<ItemStack>[])new NonNullList[] { this.mainInventory, this.armorInventory, this.offHandInventory });
  ...
  private boolean canMergeStacks(ItemStack stack1, ItemStack stack2) {
    return (!stack1.func_190926_b() && stackEqualExact(stack1, stack2) && stack1.isStackable() && stack1.func_190916_E() < stack1.getMaxStackSize() && stack1.func_190916_E() < getInventoryStackLimit());
  }
  ...
  public int getSlotFor(ItemStack stack) {
    for (int i = 0; i < this.mainInventory.size(); i++) {
      if (!((ItemStack)this.mainInventory.get(i)).func_190926_b() && stackEqualExact(stack, (ItemStack)this.mainInventory.get(i)))
        return i; 
    } 
    return -1;
  }
  ...
  public int func_194014_c(ItemStack p_194014_1_) {
    for (int i = 0; i < this.mainInventory.size(); i++) {
      ...
      if (!((ItemStack)this.mainInventory.get(i)).func_190926_b() && stackEqualExact(p_194014_1_, (ItemStack)this.mainInventory.get(i)) && !((ItemStack)this.mainInventory.get(i)).isItemDamaged() && !itemstack.isItemEnchanted() && !itemstack.hasDisplayName())
        return i; 
    } 
    return -1;
  }
  ...
  public int clearMatchingItems(@Nullable Item itemIn, int metadataIn, int removeCount, @Nullable NBTTagCompound itemNBT) {
    ...
    for (int j = 0; j < getSizeInventory(); j++) {
      ...
      if (!itemstack.func_190926_b() && (itemIn == null || itemstack.getItem() == itemIn) && (metadataIn <= -1 || itemstack.getMetadata() == metadataIn) && (itemNBT == null || NBTUtil.areNBTEquals((NBTBase)itemNBT, (NBTBase)itemstack.getTagCompound(), true))) {
        int k = (removeCount <= 0) ? itemstack.func_190916_E() : Math.min(removeCount - i, itemstack.func_190916_E());
        i += k;
        if (removeCount != 0) {
          itemstack.func_190918_g(k);
          if (itemstack.func_190926_b())
            setInventorySlotContents(j, ItemStack.field_190927_a); 
    ...
    if (!this.itemStack.func_190926_b()) {
      ...
      if (metadataIn > -1 && this.itemStack.getMetadata() != metadataIn)
        return i; 
      if (itemNBT != null && !NBTUtil.areNBTEquals((NBTBase)itemNBT, (NBTBase)this.itemStack.getTagCompound(), true))
        return i; 
  ...
  private int storePartialItemStack(ItemStack itemStackIn) {
    ...
    return (i == -1) ? itemStackIn.func_190916_E() : func_191973_d(i, itemStackIn);
  }
  ...
  private int func_191973_d(int p_191973_1_, ItemStack p_191973_2_) {
    ...
    if (j > getInventoryStackLimit() - itemstack.func_190916_E())
      j = getInventoryStackLimit() - itemstack.func_190916_E(); 
    if (j == 0)
      return i; 
    i -= j;
  ...
  public int storeItemStack(ItemStack itemStackIn) {
    if (canMergeStacks(getStackInSlot(this.currentItem), itemStackIn))
      return this.currentItem; 
    if (canMergeStacks(getStackInSlot(40), itemStackIn))
      return 40; 
    for (int i = 0; i < this.mainInventory.size(); i++) {
      if (canMergeStacks((ItemStack)this.mainInventory.get(i), itemStackIn))
        return i; 
    } 
    return -1;
  }
    ...
    for (NonNullList<ItemStack> nonnulllist : this.allInventories) {
      for (int i = 0; i < nonnulllist.size(); i++) {
        if (!((ItemStack)nonnulllist.get(i)).func_190926_b())
          ((ItemStack)nonnulllist.get(i)).updateAnimation(this.player.world, (Entity)this.player, i, (this.currentItem == i)); 
      } 
    } 
  }
  ...
  public void func_191975_a(World p_191975_1_, ItemStack p_191975_2_) {
    if (!p_191975_1_.isRemote)
      while (!p_191975_2_.func_190926_b()) {
        ...
        int j = p_191975_2_.getMaxStackSize() - getStackInSlot(i).func_190916_E();
        if (func_191971_c(i, p_191975_2_.splitStack(j)))
          ((EntityPlayerMP)this.player).connection.sendPacket((Packet)new SPacketSetSlot(-2, i, getStackInSlot(i))); 
      }  
  }
  ...
  public ItemStack decrStackSize(int index, int count) {
    ...
    return (nonNullList != null && !((ItemStack)nonNullList.get(index)).func_190926_b()) ? ItemStackHelper.getAndSplit((List)nonNullList, index, count) : ItemStack.field_190927_a;
  }
  ...
  public NBTTagList writeToNBT(NBTTagList nbtTagListIn) {
    ...
    for (int k = 0; k < this.offHandInventory.size(); k++) {
      if (!((ItemStack)this.offHandInventory.get(k)).func_190926_b()) {
        ...
        nbtTagListIn.appendTag((NBTBase)nbttagcompound2);
      } 
    } 
    return nbtTagListIn;
  }
  ...
  public boolean isUsableByPlayer(EntityPlayer player) {
    if (this.player.isDead)
      return false; 
    return (player.getDistanceSqToEntity((Entity)this.player) <= 64.0D);
  }
