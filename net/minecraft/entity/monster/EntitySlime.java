...
public class EntitySlime extends EntityLiving implements IMob {
  ...
  protected void setSlimeSize(int size, boolean p_70799_2_) {
    ...
    getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue((0.2F + 0.1F * size));
    if (p_70799_2_)
  ...
  public void setDead() {
    ...
    if (!this.world.isRemote && i > 1 && getHealth() <= 0.0F) {
      ...
      for (int k = 0; k < j; k++) {
        ...
        this.world.spawnEntityInWorld((Entity)entityslime);
      } 
    } 
    super.setDead();
  }
  ...
  protected void dealDamage(EntityLivingBase entityIn) {
    int i = getSlimeSize();
    if (canEntityBeSeen((Entity)entityIn) && getDistanceSqToEntity((Entity)entityIn) < 0.6D * i * 0.6D * i && entityIn.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), getAttackStrength())) {
      playSound(SoundEvents.ENTITY_SLIME_ATTACK, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
      applyEnchantments((EntityLivingBase)this, (Entity)entityIn);
    } 
  }
  ...
  public boolean getCanSpawnHere() {
    ...
    if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
      ...
      if (biome == Biomes.SWAMPLAND && this.posY > 50.0D && this.posY < 70.0D && this.rand.nextFloat() < 0.5F && this.rand.nextFloat() < this.world.getCurrentMoonPhaseFactor() && this.world.getLightFromNeighbors(new BlockPos((Entity)this)) <= this.rand.nextInt(8))
