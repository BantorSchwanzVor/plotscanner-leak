...
public class EntityMinecartHopper extends EntityMinecartContainer implements IHopper {
  ...
  public void onUpdate() {
    super.onUpdate();
    if (!this.world.isRemote && isEntityAlive() && getBlocked()) {
  ...
  public boolean captureDroppedItems() {
    if (TileEntityHopper.captureDroppedItems(this))
      return true; 
    List<EntityItem> list = this.world.getEntitiesWithinAABB(EntityItem.class, getEntityBoundingBox().expand(0.25D, 0.0D, 0.25D), EntitySelectors.IS_ALIVE);
    if (!list.isEmpty())
      TileEntityHopper.putDropInInventoryAllSlots(null, (IInventory)this, list.get(0)); 
    return false;
  }
