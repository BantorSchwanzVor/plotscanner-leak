...
public class EntityItem extends Entity {
  ...
  public EntityItem(World worldIn) {
    ...
    this.hoverStart = (float)(Math.random() * Math.PI * 2.0D);
    setSize(0.25F, 0.25F);
    setEntityItemStack(ItemStack.field_190927_a);
  }
  ...
  public void onUpdate() {
    if (getEntityItem().func_190926_b()) {
      setDead();
    } else {
      super.onUpdate();
      ...
      float f = 0.98F;
      if (this.onGround)
        f = (this.world.getBlockState(new BlockPos(MathHelper.floor(this.posX), MathHelper.floor((getEntityBoundingBox()).minY) - 1, MathHelper.floor(this.posZ))).getBlock()).slipperiness * 0.98F; 
      this.motionX *= f;
  ...
  private void searchForOtherItemsNearby() {
    for (EntityItem entityitem : this.world.getEntitiesWithinAABB(EntityItem.class, getEntityBoundingBox().expand(0.5D, 0.0D, 0.5D)))
  ...
  public boolean handleWaterMovement() {
    if (this.world.handleMaterialAcceleration(getEntityBoundingBox(), Material.WATER, this)) {
  ...
  public boolean attackEntityFrom(DamageSource source, float amount) {
    if (isEntityInvulnerable(source))
      return false; 
    if (!getEntityItem().func_190926_b() && getEntityItem().getItem() == Items.NETHER_STAR && source.isExplosion())
      return false; 
    setBeenAttacked();
  ...
  public static void registerFixesItem(DataFixer fixer) {
    fixer.registerWalker(FixTypes.ENTITY, (IDataWalker)new ItemStackData(EntityItem.class, new String[] { "Item" }));
  }
  ...
  public void writeEntityToNBT(NBTTagCompound compound) {
    ...
    if (!getEntityItem().func_190926_b())
      compound.setTag("Item", (NBTBase)getEntityItem().writeToNBT(new NBTTagCompound())); 
  }
