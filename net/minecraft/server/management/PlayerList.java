...
public abstract class PlayerList {
  ...
  public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn) {
    ...
    nethandlerplayserver.sendPacket((Packet)new SPacketJoinGame(playerIn.getEntityId(), playerIn.interactionManager.getGameType(), worldinfo.isHardcoreModeEnabled(), worldserver.provider.getDimensionType().getId(), worldserver.getDifficulty(), getMaxPlayers(), worldinfo.getTerrainType(), worldserver.getGameRules().getBoolean("reducedDebugInfo")));
    nethandlerplayserver.sendPacket((Packet)new SPacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer())).writeString(getServerInstance().getServerModName())));
    nethandlerplayserver.sendPacket((Packet)new SPacketServerDifficulty(worldinfo.getDifficulty(), worldinfo.isDifficultyLocked()));
    nethandlerplayserver.sendPacket((Packet)new SPacketPlayerAbilities(playerIn.capabilities));
    nethandlerplayserver.sendPacket((Packet)new SPacketHeldItemChange(playerIn.inventory.currentItem));
    ...
    sendChatMsg((ITextComponent)textcomponenttranslation);
    ...
    for (PotionEffect potioneffect : playerIn.getActivePotionEffects())
      nethandlerplayserver.sendPacket((Packet)new SPacketEntityEffect(playerIn.getEntityId(), potioneffect)); 
    if (nbttagcompound != null && nbttagcompound.hasKey("RootVehicle", 10)) {
      ...
      Entity entity1 = AnvilChunkLoader.readWorldEntity(nbttagcompound1.getCompoundTag("Entity"), (World)worldserver, true);
      if (entity1 != null) {
  ...
  protected void sendScoreboard(ServerScoreboard scoreboardIn, EntityPlayerMP playerIn) {
    ...
    for (ScorePlayerTeam scoreplayerteam : scoreboardIn.getTeams())
      playerIn.connection.sendPacket((Packet)new SPacketTeams(scoreplayerteam, 0)); 
    for (int i = 0; i < 19; i++) {
      ScoreObjective scoreobjective = scoreboardIn.getObjectiveInDisplaySlot(i);
      if (scoreobjective != null && !set.contains(scoreobjective)) {
        for (Packet<?> packet : (Iterable<Packet<?>>)scoreboardIn.getCreatePackets(scoreobjective))
  ...
  public void setPlayerManager(WorldServer[] worldServers) {
    ...
    worldServers[0].getWorldBorder().addListener(new IBorderListener() {
          public void onSizeChanged(WorldBorder border, double newSize) {
            PlayerList.this.sendPacketToAllPlayers((Packet<?>)new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_SIZE));
          }
          
          public void onTransitionStarted(WorldBorder border, double oldSize, double newSize, long time) {
            PlayerList.this.sendPacketToAllPlayers((Packet<?>)new SPacketWorldBorder(border, SPacketWorldBorder.Action.LERP_SIZE));
          }
          
          public void onCenterChanged(WorldBorder border, double x, double z) {
            PlayerList.this.sendPacketToAllPlayers((Packet<?>)new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_CENTER));
          }
          
          public void onWarningTimeChanged(WorldBorder border, int newTime) {
            PlayerList.this.sendPacketToAllPlayers((Packet<?>)new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_WARNING_TIME));
          }
          
          public void onWarningDistanceChanged(WorldBorder border, int newDistance) {
            PlayerList.this.sendPacketToAllPlayers((Packet<?>)new SPacketWorldBorder(border, SPacketWorldBorder.Action.SET_WARNING_BLOCKS));
          }
  ...
  public void playerLoggedIn(EntityPlayerMP playerIn) {
    ...
    sendPacketToAllPlayers((Packet<?>)new SPacketPlayerListItem(SPacketPlayerListItem.Action.ADD_PLAYER, new EntityPlayerMP[] { playerIn }));
    WorldServer worldserver = this.mcServer.worldServerForDimension(playerIn.dimension);
    for (int i = 0; i < this.playerEntityList.size(); i++) {
      playerIn.connection.sendPacket((Packet)new SPacketPlayerListItem(SPacketPlayerListItem.Action.ADD_PLAYER, new EntityPlayerMP[] { this.playerEntityList.get(i) }));
    } 
  ...
  public void playerLoggedOut(EntityPlayerMP playerIn) {
    ...
    sendPacketToAllPlayers((Packet<?>)new SPacketPlayerListItem(SPacketPlayerListItem.Action.REMOVE_PLAYER, new EntityPlayerMP[] { playerIn }));
  }
  ...
  public EntityPlayerMP createPlayerForUser(GameProfile profile) {
    ...
    for (EntityPlayerMP entityplayermp1 : list)
      entityplayermp1.connection.func_194028_b((ITextComponent)new TextComponentTranslation("multiplayer.disconnect.duplicate_login", new Object[0])); 
    if (this.mcServer.isDemo()) {
      playerinteractionmanager = new DemoPlayerInteractionManager((World)this.mcServer.worldServerForDimension(0));
    } else {
      playerinteractionmanager = new PlayerInteractionManager((World)this.mcServer.worldServerForDimension(0));
    } 
  ...
  public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP playerIn, int dimension, boolean conqueredEnd) {
    ...
    playerIn.getServerWorld().getEntityTracker().untrackEntity((Entity)playerIn);
    playerIn.getServerWorld().getPlayerChunkMap().removePlayer(playerIn);
    this.playerEntityList.remove(playerIn);
    this.mcServer.worldServerForDimension(playerIn.dimension).removeEntityDangerously((Entity)playerIn);
    BlockPos blockpos = playerIn.getBedLocation();
    boolean flag = playerIn.isSpawnForced();
    playerIn.dimension = dimension;
    if (this.mcServer.isDemo()) {
      playerinteractionmanager = new DemoPlayerInteractionManager((World)this.mcServer.worldServerForDimension(playerIn.dimension));
    } else {
      playerinteractionmanager = new PlayerInteractionManager((World)this.mcServer.worldServerForDimension(playerIn.dimension));
    } 
    ...
    while (!worldserver.getCollisionBoxes((Entity)entityplayermp, entityplayermp.getEntityBoundingBox()).isEmpty() && entityplayermp.posY < 256.0D)
  ...
  public void updatePermissionLevel(EntityPlayerMP player) {
    ...
    int i = canSendCommands(gameprofile) ? this.ops.getPermissionLevel(gameprofile) : 0;
  ...
  public void changePlayerDimension(EntityPlayerMP player, int dimensionIn) {
    ...
    player.connection.sendPacket((Packet)new SPacketRespawn(player.dimension, player.world.getDifficulty(), player.world.getWorldInfo().getTerrainType(), player.interactionManager.getGameType()));
    updatePermissionLevel(player);
    worldserver.removeEntityDangerously((Entity)player);
    player.isDead = false;
    transferEntityToWorld((Entity)player, i, worldserver, worldserver1);
  ...
  public void onTick() {
    if (++this.playerPingIndex > 600) {
      sendPacketToAllPlayers((Packet<?>)new SPacketPlayerListItem(SPacketPlayerListItem.Action.UPDATE_LATENCY, this.playerEntityList));
  ...
  private void sendPlayerPermissionLevel(EntityPlayerMP player, int permLevel) {
    if (player != null && player.connection != null) {
      ...
      player.connection.sendPacket((Packet)new SPacketEntityStatus((Entity)player, b0));
    } 
  }
  ...
  public void updateTimeAndWeatherForPlayer(EntityPlayerMP playerIn, WorldServer worldIn) {
    ...
    playerIn.connection.sendPacket((Packet)new SPacketWorldBorder(worldborder, SPacketWorldBorder.Action.INITIALIZE));
    playerIn.connection.sendPacket((Packet)new SPacketTimeUpdate(worldIn.getTotalWorldTime(), worldIn.getWorldTime(), worldIn.getGameRules().getBoolean("doDaylightCycle")));
    BlockPos blockpos = worldIn.getSpawnPoint();
    playerIn.connection.sendPacket((Packet)new SPacketSpawnPosition(blockpos));
    if (worldIn.isRaining()) {
      playerIn.connection.sendPacket((Packet)new SPacketChangeGameState(1, 0.0F));
      playerIn.connection.sendPacket((Packet)new SPacketChangeGameState(7, worldIn.getRainStrength(1.0F)));
      playerIn.connection.sendPacket((Packet)new SPacketChangeGameState(8, worldIn.getThunderStrength(1.0F)));
    } 
  }
  ...
  public void syncPlayerInventory(EntityPlayerMP playerIn) {
    ...
    playerIn.connection.sendPacket((Packet)new SPacketHeldItemChange(playerIn.inventory.currentItem));
  }
  ...
  public void removeAllPlayers() {
    for (int i = 0; i < this.playerEntityList.size(); i++)
      ((EntityPlayerMP)this.playerEntityList.get(i)).connection.func_194028_b((ITextComponent)new TextComponentTranslation("multiplayer.disconnect.server_shutdown", new Object[0])); 
  }
  ...
  public void sendChatMsgImpl(ITextComponent component, boolean isSystem) {
    ...
    sendPacketToAllPlayers((Packet<?>)new SPacketChat(component, chattype));
  }
