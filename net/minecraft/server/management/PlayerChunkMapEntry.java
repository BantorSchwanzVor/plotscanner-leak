...
public class PlayerChunkMapEntry {
  ...
  public void removePlayer(EntityPlayerMP player) {
    if (this.players.contains(player)) {
      if (this.sentToPlayers)
        player.connection.sendPacket((Packet)new SPacketUnloadChunk(this.pos.chunkXPos, this.pos.chunkZPos)); 
  ...
  public boolean sendToPlayers() {
    ...
    for (EntityPlayerMP entityplayermp : this.players) {
      entityplayermp.connection.sendPacket((Packet)sPacketChunkData);
  ...
  public void sendNearbySpecialEntities(EntityPlayerMP player) {
    if (this.sentToPlayers) {
      player.connection.sendPacket((Packet)new SPacketChunkData(this.chunk, 65535));
    ...
    if (this.sentToPlayers && this.chunk != null)
      if (this.changes != 0) {
        if (this.changes == 1) {
          ...
          sendPacket((Packet<?>)new SPacketBlockChange((World)this.playerChunkMap.getWorldServer(), blockpos));
          if (this.playerChunkMap.getWorldServer().getBlockState(blockpos).getBlock().hasTileEntity())
            sendBlockEntity(this.playerChunkMap.getWorldServer().getTileEntity(blockpos)); 
        } else if (this.changes == 64) {
          sendPacket((Packet<?>)new SPacketChunkData(this.chunk, this.changedSectionFilter));
        } else {
          sendPacket((Packet<?>)new SPacketMultiBlockChange(this.changes, this.changedBlocks, this.chunk));
          for (int l = 0; l < this.changes; l++) {
            ...
            if (this.playerChunkMap.getWorldServer().getBlockState(blockpos1).getBlock().hasTileEntity())
              sendBlockEntity(this.playerChunkMap.getWorldServer().getTileEntity(blockpos1)); 
          } 
        } 
        this.changes = 0;
        this.changedSectionFilter = 0;
  ...
  private void sendBlockEntity(@Nullable TileEntity be) {
    if (be != null) {
      ...
      if (spacketupdatetileentity != null)
        sendPacket((Packet<?>)spacketupdatetileentity); 
    } 
  }
  ...
  public boolean hasPlayerMatchingInRange(double range, Predicate<EntityPlayerMP> predicate) {
    ...
    for (int j = this.players.size(); i < j; i++) {
      ...
      if (predicate.apply(entityplayermp) && this.pos.getDistanceSq((Entity)entityplayermp) < range * range)
        return true; 
    } 
    return false;
  }
  ...
  public double getClosestPlayerDistance() {
    ...
    for (EntityPlayerMP entityplayermp : this.players) {
      double d1 = this.pos.getDistanceSq((Entity)entityplayermp);
      if (d1 < d0)
        d0 = d1; 
    } 
    return d0;
