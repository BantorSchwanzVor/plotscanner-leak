...
public class TileEntityPiston extends TileEntity implements ITickable {
  ...
  public float getOffsetX(float ticks) {
    return this.pistonFacing.getFrontOffsetX() * getExtendedProgress(getProgress(ticks));
  }
  
  public float getOffsetY(float ticks) {
    return this.pistonFacing.getFrontOffsetY() * getExtendedProgress(getProgress(ticks));
  }
  
  public float getOffsetZ(float ticks) {
    return this.pistonFacing.getFrontOffsetZ() * getExtendedProgress(getProgress(ticks));
  }
  ...
  public AxisAlignedBB getAABB(IBlockAccess p_184321_1_, BlockPos p_184321_2_) {
    return getAABB(p_184321_1_, p_184321_2_, this.progress).union(getAABB(p_184321_1_, p_184321_2_, this.lastProgress));
  }
  ...
  private void moveCollidedEntities(float p_184322_1_) {
    ...
    func_190606_j().addCollisionBoxToList(this.world, BlockPos.ORIGIN, new AxisAlignedBB(BlockPos.ORIGIN), list, null, true);
    if (!list.isEmpty()) {
      AxisAlignedBB axisalignedbb = func_190607_a(func_191515_a(list));
      List<Entity> list1 = this.world.getEntitiesWithinAABBExcludingEntity(null, func_190610_a(axisalignedbb, enumfacing, d0).union(axisalignedbb));
      if (!list1.isEmpty()) {
  ...
  private AxisAlignedBB func_190607_a(AxisAlignedBB p_190607_1_) {
    double d0 = getExtendedProgress(this.progress);
  ...
  public void func_190609_a(World p_190609_1_, BlockPos p_190609_2_, AxisAlignedBB p_190609_3_, List<AxisAlignedBB> p_190609_4_, @Nullable Entity p_190609_5_) {
    if (!this.extending && this.shouldHeadBeRendered)
      this.pistonState.withProperty((IProperty)BlockPistonBase.EXTENDED, Boolean.valueOf(true)).addCollisionBoxToList(p_190609_1_, p_190609_2_, p_190609_3_, p_190609_4_, p_190609_5_, false); 
    ...
    if (this.progress >= 1.0D || enumfacing != (this.extending ? this.pistonFacing : this.pistonFacing.getOpposite())) {
      ...
      if (shouldPistonHeadBeRendered()) {
        iblockstate = Blocks.PISTON_HEAD.getDefaultState().withProperty((IProperty)BlockPistonExtension.FACING, (Comparable)this.pistonFacing).withProperty((IProperty)BlockPistonExtension.SHORT, Boolean.valueOf(this.extending ^ ((1.0F - this.progress < 0.25F))));
      } else {
        iblockstate = this.pistonState;
      } 
      float f = getExtendedProgress(this.progress);
