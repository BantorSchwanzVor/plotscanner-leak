...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, ISidedInventory {
  ...
  private void updateSegmentColors() {
    ...
    for (int i1 = j + 1; i1 < 256; ) {
      float[] afloat;
      IBlockState iblockstate = this.world.getBlockState((BlockPos)blockpos$mutableblockpos.setPos(i, i1, k));
      if (iblockstate.getBlock() == Blocks.STAINED_GLASS) {
        afloat = ((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlass.COLOR)).func_193349_f();
      } else {
        if (iblockstate.getBlock() != Blocks.STAINED_GLASS_PANE) {
          ...
          afloat = ((EnumDyeColor)iblockstate.getValue((IProperty)BlockStainedGlassPane.COLOR)).func_193349_f();
          if (!flag)
  ...
  static {
    ...
    for (i = (arrayOfPotion = EFFECTS_LIST).length, b = 0; b < i; ) {
      Potion[] apotion = arrayOfPotion[b];
