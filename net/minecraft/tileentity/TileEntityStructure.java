...
public class TileEntityStructure extends TileEntity {
  ...
  private void updateBlockState() {
    if (this.world != null) {
      ...
      if (iblockstate.getBlock() == Blocks.STRUCTURE_BLOCK)
        this.world.setBlockState(blockpos, iblockstate.withProperty((IProperty)BlockStructure.MODE, this.mode), 2); 
    } 
  }
  
  @Nullable
  public SPacketUpdateTileEntity getUpdatePacket() {
    return new SPacketUpdateTileEntity(this.pos, 7, getUpdateTag());
  }
  ...
  public void setMode(Mode modeIn) {
    ...
    if (iblockstate.getBlock() == Blocks.STRUCTURE_BLOCK)
      this.world.setBlockState(getPos(), iblockstate.withProperty((IProperty)BlockStructure.MODE, modeIn), 2); 
  }
  ...
  private List<TileEntityStructure> getNearbyCornerBlocks(BlockPos p_184418_1_, BlockPos p_184418_2_) {
    ...
    for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(p_184418_1_, p_184418_2_)) {
      ...
      if (iblockstate.getBlock() == Blocks.STRUCTURE_BLOCK) {
        TileEntity tileentity = this.world.getTileEntity((BlockPos)blockpos$mutableblockpos);
  ...
  private StructureBoundingBox calculateEnclosingBoundingBox(BlockPos p_184416_1_, List<TileEntityStructure> p_184416_2_) {
    ...
    for (TileEntityStructure tileentitystructure : p_184416_2_) {
      ...
      if (blockpos1.getZ() > structureboundingbox.maxZ)
  ...
  public boolean save(boolean p_189712_1_) {
    if (this.mode == Mode.SAVE && !this.world.isRemote && !StringUtils.isNullOrEmpty(this.name)) {
      BlockPos blockpos = getPos().add((Vec3i)this.position);
  ...
  public boolean load(boolean p_189714_1_) {
    if (this.mode == Mode.LOAD && !this.world.isRemote && !StringUtils.isNullOrEmpty(this.name)) {
      BlockPos blockpos = getPos();
      BlockPos blockpos1 = blockpos.add((Vec3i)this.position);
      WorldServer worldserver = (WorldServer)this.world;
      MinecraftServer minecraftserver = this.world.getMinecraftServer();
      TemplateManager templatemanager = worldserver.getStructureTemplateManager();
  ...
  public ITextComponent getDisplayName() {
    return (ITextComponent)new TextComponentTranslation("structure_block.hover." + this.mode.modeName, new Object[] { (this.mode == Mode.DATA) ? this.metadata : this.name });
  }
  ...
  public enum Mode implements IStringSerializable {
    ...
    static {
      ...
      for (i = (arrayOfMode = values()).length, b = 0; b < i; ) {
        Mode tileentitystructure$mode = arrayOfMode[b];
