...
public abstract class CommandBlockBaseLogic implements ICommandSender {
  ...
  public ITextComponent getLastOutput() {
    return (this.lastOutput == null) ? (ITextComponent)new TextComponentString("") : this.lastOutput;
  }
  ...
  public void readDataFromNBT(NBTTagCompound nbt) {
    ...
    if (nbt.hasKey("LastOutput", 8) && this.trackOutput) {
      try {
        ...
        this.lastOutput = (ITextComponent)new TextComponentString(throwable.getMessage());
      } 
    } else {
  ...
  public boolean trigger(World worldIn) {
    if (!worldIn.isRemote && worldIn.getTotalWorldTime() != this.field_193041_b) {
      if ("Searge".equalsIgnoreCase(this.commandStored)) {
        this.lastOutput = (ITextComponent)new TextComponentString("#itzlipofutzli");
        this.successCount = 1;
        return true;
      } 
  ...
  public void setCommandStat(CommandResultStats.Type type, int amount) {
    this.resultStats.setCommandStatForSender(getServer(), this, type, amount);
  }
