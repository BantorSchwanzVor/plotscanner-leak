...
public class TileEntityFurnace extends TileEntityLockable implements ITickable, ISidedInventory {
  ...
  public ItemStack decrStackSize(int index, int count) {
    return ItemStackHelper.getAndSplit((List)this.furnaceItemStacks, index, count);
  }
  
  public ItemStack removeStackFromSlot(int index) {
    return ItemStackHelper.getAndRemove((List)this.furnaceItemStacks, index);
  }
  ...
  public static void registerFixesFurnace(DataFixer fixer) {
    fixer.registerWalker(FixTypes.BLOCK_ENTITY, (IDataWalker)new ItemStackDataLists(TileEntityFurnace.class, new String[] { "Items" }));
  }
  
  public void readFromNBT(NBTTagCompound compound) {
    super.readFromNBT(compound);
    this.furnaceItemStacks = NonNullList.func_191197_a(getSizeInventory(), ItemStack.field_190927_a);
  ...
  private boolean canSmelt() {
    ...
    if (!itemstack1.isItemEqual(itemstack))
      return false; 
    if (itemstack1.func_190916_E() < getInventoryStackLimit() && itemstack1.func_190916_E() < itemstack1.getMaxStackSize())
      return true; 
  ...
  public static int getItemBurnTime(ItemStack stack) {
    ...
    if (item == Item.getItemFromBlock((Block)Blocks.WOODEN_SLAB))
      return 150; 
    ...
    if (item != Items.BOW && item != Items.FISHING_ROD) {
      ...
      if (item != Item.getItemFromBlock(Blocks.SAPLING) && item != Items.BOWL) {
        if (item == Items.BLAZE_ROD)
          return 2400; 
        if (item instanceof net.minecraft.item.ItemDoor && item != Items.IRON_DOOR)
          return 200; 
        return (item instanceof net.minecraft.item.ItemBoat) ? 400 : 0;
      } 
      return 100;
    } 
    return 300;
