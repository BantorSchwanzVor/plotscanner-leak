...
public class BlockGrass extends Block implements IGrowable {
    ...
    if (!worldIn.isRemote)
      if (worldIn.getLightFromNeighbors(pos.up()) < 4 && worldIn.getBlockState(pos.up()).getLightOpacity() > 2) {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          if (iblockstate1.getBlock() == Blocks.DIRT && iblockstate1.getValue((IProperty)BlockDirt.VARIANT) == BlockDirt.DirtType.DIRT && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity() <= 2)
  ...
  protected BlockStateContainer createBlockState() {
    return new BlockStateContainer(this, new IProperty[] { (IProperty)SNOWY });
  }
}
