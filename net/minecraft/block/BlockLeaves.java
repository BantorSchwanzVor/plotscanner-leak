      ...
      for (int j1 = -1; j1 <= 1; j1++) {
        for (int k1 = -1; k1 <= 1; k1++) {
          for (int l1 = -1; l1 <= 1; l1++) {
            ...
            if (iblockstate.getMaterial() == Material.LEAVES && !((Boolean)iblockstate.getValue((IProperty)CHECK_DECAY)).booleanValue())
          ...
          for (int i2 = -4; i2 <= 4; i2++) {
            for (int j2 = -4; j2 <= 4; j2++) {
              for (int k2 = -4; k2 <= 4; k2++) {
                IBlockState iblockstate = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.setPos(k + i2, l + j2, i1 + k2));
