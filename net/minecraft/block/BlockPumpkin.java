...
public class BlockPumpkin extends BlockHorizontal {
  ...
  private void trySpawnGolem(World worldIn, BlockPos pos) {
    ...
    if (blockpattern$patternhelper != null) {
      ...
      for (EntityPlayerMP entityplayermp : worldIn.getEntitiesWithinAABB(EntityPlayerMP.class, entitysnowman.getEntityBoundingBox().expandXyz(5.0D)))
        CriteriaTriggers.field_192133_m.func_192229_a(entityplayermp, (Entity)entitysnowman); 
      for (int l = 0; l < 120; l++)
  ...
  protected BlockPattern getSnowmanBasePattern() {
    if (this.snowmanBasePattern == null)
      this.snowmanBasePattern = FactoryBlockPattern.start().aisle(new String[] { " ", "#", "#" }).where('#', BlockWorldState.hasState((Predicate)BlockStateMatcher.forBlock(Blocks.SNOW))).build(); 
    return this.snowmanBasePattern;
  }
  
  protected BlockPattern getSnowmanPattern() {
    if (this.snowmanPattern == null)
      this.snowmanPattern = FactoryBlockPattern.start().aisle(new String[] { "^", "#", "#" }).where('^', BlockWorldState.hasState(IS_PUMPKIN)).where('#', BlockWorldState.hasState((Predicate)BlockStateMatcher.forBlock(Blocks.SNOW))).build(); 
    return this.snowmanPattern;
  }
  
  protected BlockPattern getGolemBasePattern() {
    if (this.golemBasePattern == null)
      this.golemBasePattern = FactoryBlockPattern.start().aisle(new String[] { "~ ~", "###", "~#~" }).where('#', BlockWorldState.hasState((Predicate)BlockStateMatcher.forBlock(Blocks.IRON_BLOCK))).where('~', BlockWorldState.hasState((Predicate)BlockMaterialMatcher.forMaterial(Material.AIR))).build(); 
    return this.golemBasePattern;
  }
  
  protected BlockPattern getGolemPattern() {
    if (this.golemPattern == null)
      this.golemPattern = FactoryBlockPattern.start().aisle(new String[] { "~^~", "###", "~#~" }).where('^', BlockWorldState.hasState(IS_PUMPKIN)).where('#', BlockWorldState.hasState((Predicate)BlockStateMatcher.forBlock(Blocks.IRON_BLOCK))).where('~', BlockWorldState.hasState((Predicate)BlockMaterialMatcher.forMaterial(Material.AIR))).build(); 
