...
public class BlockBed extends BlockHorizontal implements ITileEntityProvider {
    ...
    if (worldIn.provider.canRespawnHere() && worldIn.getBiome(pos) != Biomes.HELL) {
      if (((Boolean)state.getValue((IProperty)OCCUPIED)).booleanValue()) {
        ...
        if (entityplayer != null) {
          playerIn.addChatComponentMessage((ITextComponent)new TextComponentTranslation("tile.bed.occupied", new Object[0]), true);
          return true;
        } 
      ...
      if (entityplayer$sleepresult == EntityPlayer.SleepResult.NOT_POSSIBLE_NOW) {
        playerIn.addChatComponentMessage((ITextComponent)new TextComponentTranslation("tile.bed.noSleep", new Object[0]), true);
      } else if (entityplayer$sleepresult == EntityPlayer.SleepResult.NOT_SAFE) {
        playerIn.addChatComponentMessage((ITextComponent)new TextComponentTranslation("tile.bed.notSafe", new Object[0]), true);
      } else if (entityplayer$sleepresult == EntityPlayer.SleepResult.TOO_FAR_AWAY) {
        playerIn.addChatComponentMessage((ITextComponent)new TextComponentTranslation("tile.bed.tooFarAway", new Object[0]), true);
      } 
      return true;
    } 
  ...
  public Item getItemDropped(IBlockState state, Random rand, int fortune) {
    return (state.getValue((IProperty)PART) == EnumPartType.FOOT) ? Items.field_190931_a : Items.BED;
  }
  ...
  public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
    ...
    if (state.getValue((IProperty)PART) == EnumPartType.FOOT)
      blockpos = pos.offset((EnumFacing)state.getValue((IProperty)FACING)); 
  ...
  public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
    if (player.capabilities.isCreativeMode && state.getValue((IProperty)PART) == EnumPartType.FOOT) {
      BlockPos blockpos = pos.offset((EnumFacing)state.getValue((IProperty)FACING));
      if (worldIn.getBlockState(blockpos).getBlock() == this)
        worldIn.setBlockToAir(blockpos); 
    } 
  }
  ...
  public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, TileEntity te, ItemStack stack) {
    if (state.getValue((IProperty)PART) == EnumPartType.HEAD && te instanceof TileEntityBed) {
  ...
  protected BlockStateContainer createBlockState() {
    return new BlockStateContainer(this, new IProperty[] { (IProperty)FACING, (IProperty)PART, (IProperty)OCCUPIED });
  }
  
  public TileEntity createNewTileEntity(World worldIn, int meta) {
    return (TileEntity)new TileEntityBed();
  }
  
  public static boolean func_193385_b(int p_193385_0_) {
    return ((p_193385_0_ & 0x8) != 0);
  }
