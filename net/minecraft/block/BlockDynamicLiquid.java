    ...
    if (i > 0) {
      ...
      if (this.adjacentSourceBlocks >= 2 && this.blockMaterial == Material.WATER) {
        ...
        if (iblockstate.getMaterial().isSolid()) {
          i1 = 0;
        } else if (iblockstate.getMaterial() == this.blockMaterial && ((Integer)iblockstate.getValue((IProperty)LEVEL)).intValue() == 0) {
          i1 = 0;
        } 
      } 
  ...
  private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
    ...
    for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
      if (enumfacing != calculateFlowCost) {
        ...
        if (!isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getMaterial() != this.blockMaterial || ((Integer)iblockstate.getValue((IProperty)LEVEL)).intValue() > 0)) {
