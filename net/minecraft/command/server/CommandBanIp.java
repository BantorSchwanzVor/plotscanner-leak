...
public class CommandBanIp extends CommandBase {
  ...
  protected void banIp(MinecraftServer server, ICommandSender sender, String ipAddress, @Nullable String banReason) {
    ...
    server.getPlayerList().getBannedIPs().addEntry((UserListEntry)userlistipbansentry);
    List<EntityPlayerMP> list = server.getPlayerList().getPlayersMatchingAddress(ipAddress);
    String[] astring = new String[list.size()];
    int i = 0;
    for (EntityPlayerMP entityplayermp : list) {
      entityplayermp.connection.func_194028_b((ITextComponent)new TextComponentTranslation("multiplayer.disconnect.ip_banned", new Object[0]));
      astring[i++] = entityplayermp.getName();
    } 
    if (list.isEmpty()) {
      notifyCommandListener(sender, (ICommand)this, "commands.banip.success", new Object[] { ipAddress });
    } else {
      notifyCommandListener(sender, (ICommand)this, "commands.banip.success.players", new Object[] { ipAddress, joinNiceString((Object[])astring) });
    } 
  }
}
