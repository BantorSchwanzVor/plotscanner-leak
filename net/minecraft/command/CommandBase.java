...
public abstract class CommandBase implements ICommand {
  ...
  public static NBTTagCompound entityToNBT(Entity theEntity) {
    ...
    if (theEntity instanceof EntityPlayer) {
      ...
      if (!itemstack.func_190926_b())
        nbttagcompound.setTag("SelectedItem", (NBTBase)itemstack.writeToNBT(new NBTTagCompound())); 
    } 
  ...
  public static <T extends Entity> T getEntity(MinecraftServer server, ICommandSender sender, String target, Class<? extends T> targetClass) throws EntityNotFoundException, CommandException {
    ...
    if (entityPlayerMP == null)
      try {
        ...
        Entity entity1 = server.getEntityFromUuid(uuid);
        if (entity1 == null)
          entityPlayerMP = server.getPlayerList().getPlayerByUUID(uuid); 
      } catch (IllegalArgumentException var6) {
        if ((target.split("-")).length == 5)
          throw new EntityNotFoundException("commands.generic.entity.invalidUuid", new Object[] { target }); 
      }  
    if (entityPlayerMP != null && targetClass.isAssignableFrom(entityPlayerMP.getClass()))
  ...
  public static List<Entity> getEntityList(MinecraftServer server, ICommandSender sender, String target) throws EntityNotFoundException, CommandException {
    return EntitySelector.hasArguments(target) ? EntitySelector.<Entity>matchEntities(sender, target, Entity.class) : Lists.newArrayList((Object[])new Entity[] { getEntity(server, sender, target) });
  }
  ...
  public static ITextComponent getChatComponentFromNthArg(ICommandSender sender, String[] args, int index, boolean p_147176_3_) throws PlayerNotFoundException, CommandException {
    ...
    for (int i = index; i < args.length; i++) {
      ...
      TextComponentString textComponentString1 = new TextComponentString(args[i]);
      if (p_147176_3_) {
    ...
    try {
      ...
      try {
        ...
        for (Map.Entry<IProperty<?>, Comparable<?>> entry : map.entrySet())
          iblockstate = func_190793_a(iblockstate, (IProperty<Comparable>)entry.getKey(), entry.getValue()); 
        return iblockstate;
        ...
        return new Predicate<IBlockState>() {
            public boolean apply(@Nullable IBlockState p_apply_1_) {
              if (p_apply_1_ != null && p_190791_0_ == p_apply_1_.getBlock()) {
                for (Map.Entry<IProperty<?>, Comparable<?>> entry : (Iterable<Map.Entry<IProperty<?>, Comparable<?>>>)map.entrySet()) {
  ...
  private static Map<IProperty<?>, Comparable<?>> func_190795_c(Block p_190795_0_, String p_190795_1_) throws InvalidBlockStateException {
    ...
    while (true) {
      ...
      if (iproperty == null || !iterator1.hasNext())
        break; 
      Comparable<?> comparable = (Comparable<?>)func_190792_a(iproperty, iterator1.next());
      if (comparable == null)
        break; 
    ...
    for (int i = 0; i < components.size(); i++) {
      if (i > 0)
        if (i == components.size() - 1) {
          textComponentString.appendText(" and ");
        } else if (i > 0) {
          textComponentString.appendText(", ");
        }  
      textComponentString.appendSibling(components.get(i));
    } 
    return (ITextComponent)textComponentString;
  }
