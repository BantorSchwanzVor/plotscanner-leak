...
public class BiomeMesa extends Biome {
  protected static final IBlockState COARSE_DIRT = Blocks.DIRT.getDefaultState().withProperty((IProperty)BlockDirt.VARIANT, (Comparable)BlockDirt.DirtType.COARSE_DIRT);
  ...
  protected static final IBlockState ORANGE_STAINED_HARDENED_CLAY = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.ORANGE);
  
  protected static final IBlockState RED_SAND = Blocks.SAND.getDefaultState().withProperty((IProperty)BlockSand.VARIANT, (Comparable)BlockSand.EnumType.RED_SAND);
  ...
  protected BiomeDecorator createBiomeDecorator() {
    return new Decorator(null);
  }
  
  public WorldGenAbstractTree genBigTreeChance(Random rand) {
    return (WorldGenAbstractTree)TREE_FEATURE;
  }
  ...
  private void generateBands(long p_150619_1_) {
    ...
    Arrays.fill((Object[])this.clayBands, HARDENED_CLAY);
    ...
    for (int i = 0; i < i2; i++) {
      ...
      for (int l = 0; k + l < 64 && l < j; l++)
        this.clayBands[k + l] = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.YELLOW); 
    } 
    ...
    for (int k2 = 0; k2 < j2; k2++) {
      ...
      for (int i1 = 0; l3 + i1 < 64 && i1 < i3; i1++)
        this.clayBands[l3 + i1] = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.BROWN); 
    } 
    ...
    for (int j3 = 0; j3 < l2; j3++) {
      ...
      for (int j1 = 0; k4 + j1 < 64 && j1 < i4; j1++)
        this.clayBands[k4 + j1] = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.RED); 
    } 
    ...
    for (int l4 = 0; l4 < k3; l4++) {
      ...
      for (int k1 = 0; j4 + k1 < 64 && k1 < 1; k1++) {
        this.clayBands[j4 + k1] = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.WHITE);
        if (j4 + k1 > 1 && random.nextBoolean())
          this.clayBands[j4 + k1 - 1] = STAINED_HARDENED_CLAY.withProperty((IProperty)BlockColored.COLOR, (Comparable)EnumDyeColor.SILVER); 
  ...
  class Decorator extends BiomeDecorator {
    ...
    protected void generateOres(World worldIn, Random random) {
      ...
      genStandardOre1(worldIn, random, 20, this.goldGen, 32, 80);
    }
  }
}
