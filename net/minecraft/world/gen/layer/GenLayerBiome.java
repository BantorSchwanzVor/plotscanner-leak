      ...
      for (int j = 0; j < areaWidth; j++) {
        ...
        if (this.settings != null && this.settings.fixedBiome >= 0) {
          ...
          if (l > 0) {
            ...
            aint1[j + i * areaWidth] = Biome.getIdForBiome(this.warmBiomes[nextInt(this.warmBiomes.length)]);
          } 
        } else if (k == 2) {
          if (l > 0) {
            aint1[j + i * areaWidth] = Biome.getIdForBiome(Biomes.JUNGLE);
          } else {
            aint1[j + i * areaWidth] = Biome.getIdForBiome(this.mediumBiomes[nextInt(this.mediumBiomes.length)]);
          } 
        } else if (k == 3) {
          if (l > 0) {
            aint1[j + i * areaWidth] = Biome.getIdForBiome(Biomes.REDWOOD_TAIGA);
          } else {
            aint1[j + i * areaWidth] = Biome.getIdForBiome(this.coldBiomes[nextInt(this.coldBiomes.length)]);
          } 
        } else if (k == 4) {
          aint1[j + i * areaWidth] = Biome.getIdForBiome(this.iceBiomes[nextInt(this.iceBiomes.length)]);
        } else {
