...
public class WorldGenCanopyTree extends WorldGenAbstractTree {
  private static final IBlockState DARK_OAK_LOG = Blocks.LOG2.getDefaultState().withProperty((IProperty)BlockNewLog.VARIANT, (Comparable)BlockPlanks.EnumType.DARK_OAK);
  
  private static final IBlockState DARK_OAK_LEAVES = Blocks.LEAVES2.getDefaultState().withProperty((IProperty)BlockNewLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.DARK_OAK).withProperty((IProperty)BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
  ...
  public boolean generate(World worldIn, Random rand, BlockPos position) {
    ...
    if (k >= 1 && k + i + 1 < 256) {
      ...
      if (block != Blocks.GRASS && block != Blocks.DIRT)
        return false; 
      if (!placeTreeOfHeight(worldIn, position, i))
        return false; 
    ...
    for (int l = 0; l <= height + 1; l++) {
      ...
      for (int j1 = -i1; j1 <= i1; j1++) {
        for (int k1 = -i1; k1 <= i1; k1++) {
          if (!canGrowInto(worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.setPos(i + j1, j + l, k + k1)).getBlock()))
            return false; 
        } 
      } 
    } 
