...
public class WorldGenSwamp extends WorldGenAbstractTree {
  private static final IBlockState TRUNK = Blocks.LOG.getDefaultState().withProperty((IProperty)BlockOldLog.VARIANT, (Comparable)BlockPlanks.EnumType.OAK);
  
  private static final IBlockState LEAF = Blocks.LEAVES.getDefaultState().withProperty((IProperty)BlockOldLeaf.VARIANT, (Comparable)BlockPlanks.EnumType.OAK).withProperty((IProperty)BlockOldLeaf.CHECK_DECAY, Boolean.valueOf(false));
      ...
      for (int j = position.getY(); j <= position.getY() + 1 + i; j++) {
        ...
        for (int l = position.getX() - k; l <= position.getX() + k && flag; l++) {
          for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; i1++) {
            if (j >= 0 && j < 256) {
              IBlockState iblockstate = worldIn.getBlockState((BlockPos)blockpos$mutableblockpos.setPos(l, j, i1));
  ...
  private void addVine(World worldIn, BlockPos pos, PropertyBool prop) {
    IBlockState iblockstate = Blocks.VINE.getDefaultState().withProperty((IProperty)prop, Boolean.valueOf(true));
