...
public class WorldGenBigTree extends WorldGenAbstractTree {
  ...
  void generateLeafNode(BlockPos pos) {
    for (int i = 0; i < this.leafDistanceLimit; i++)
      crosSection(pos.up(i), leafSize(i), Blocks.LEAVES.getDefaultState().withProperty((IProperty)BlockLeaves.CHECK_DECAY, Boolean.valueOf(false))); 
  }
  ...
  int checkBlockLine(BlockPos posOne, BlockPos posTwo) {
    ...
    for (int j = 0; j <= i; j++) {
      ...
      if (!canGrowInto(this.world.getBlockState(blockpos1).getBlock()))
        return j; 
    } 
    return -1;
  }
