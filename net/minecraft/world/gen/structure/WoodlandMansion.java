...
public class WoodlandMansion extends MapGenStructure {
  ...
  public static final List<Biome> field_191072_a = Arrays.asList(new Biome[] { Biomes.ROOFED_FOREST, Biomes.MUTATED_ROOFED_FOREST });
  ...
  public static class Start extends StructureStart {
    ...
    private void func_191092_a(World p_191092_1_, ChunkGeneratorOverworld p_191092_2_, Random p_191092_3_, int p_191092_4_, int p_191092_5_) {
      ...
      if (k1 < 60) {
        ...
        this.components.addAll((Collection)list);
        updateBoundingBox();
    ...
    public void generateStructure(World worldIn, Random rand, StructureBoundingBox structurebb) {
      ...
      for (int j = structurebb.minX; j <= structurebb.maxX; j++) {
        for (int k = structurebb.minZ; k <= structurebb.maxZ; k++) {
          ...
          if (!worldIn.isAirBlock(blockpos) && this.boundingBox.isVecInside((Vec3i)blockpos)) {
            boolean flag = false;
            for (StructureComponent structurecomponent : this.components) {
              if (structurecomponent.boundingBox.isVecInside((Vec3i)blockpos)) {
                flag = true;
                break;
              } 
            } 
