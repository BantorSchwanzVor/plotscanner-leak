...
public class Template {
  ...
  public void takeBlocksFromWorld(World worldIn, BlockPos startPos, BlockPos endPos, boolean takeEntities, @Nullable Block toIgnore) {
    if (endPos.getX() >= 1 && endPos.getY() >= 1 && endPos.getZ() >= 1) {
      BlockPos blockpos = startPos.add((Vec3i)endPos).add(-1, -1, -1);
      ...
      for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos1, blockpos2)) {
        BlockPos blockpos3 = blockpos$mutableblockpos.subtract((Vec3i)blockpos1);
        ...
        if (toIgnore == null || toIgnore != iblockstate.getBlock()) {
          TileEntity tileentity = worldIn.getTileEntity((BlockPos)blockpos$mutableblockpos);
          if (tileentity != null) {
  ...
  private void takeEntitiesFromWorld(World worldIn, BlockPos startPos, BlockPos endPos) {
    ...
    for (Entity entity : list) {
      ...
      if (entity instanceof EntityPainting) {
        blockpos = ((EntityPainting)entity).getHangingPosition().subtract((Vec3i)startPos);
      } else {
  ...
  public Map<BlockPos, String> getDataBlocks(BlockPos pos, PlacementSettings placementIn) {
    ...
    for (BlockInfo template$blockinfo : this.blocks) {
      BlockPos blockpos = transformedBlockPos(placementIn, template$blockinfo.pos).add((Vec3i)pos);
      if (structureboundingbox == null || structureboundingbox.isVecInside((Vec3i)blockpos)) {
  ...
  public BlockPos calculateConnectedPos(PlacementSettings placementIn, BlockPos p_186262_2_, PlacementSettings p_186262_3_, BlockPos p_186262_4_) {
    ...
    return blockpos.subtract((Vec3i)blockpos1);
  }
  ...
  public void addBlocksToWorld(World p_189960_1_, BlockPos p_189960_2_, @Nullable ITemplateProcessor p_189960_3_, PlacementSettings p_189960_4_, int p_189960_5_) {
    if ((!this.blocks.isEmpty() || (!p_189960_4_.getIgnoreEntities() && !this.entities.isEmpty())) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
      ...
      for (BlockInfo template$blockinfo : this.blocks) {
        BlockPos blockpos = transformedBlockPos(p_189960_4_, template$blockinfo.pos).add((Vec3i)p_189960_2_);
        ...
        if (template$blockinfo1 != null) {
          ...
          if ((block == null || block != block1) && (!p_189960_4_.getIgnoreStructureBlock() || block1 != Blocks.STRUCTURE_BLOCK) && (structureboundingbox == null || structureboundingbox.isVecInside((Vec3i)blockpos))) {
      ...
      for (BlockInfo template$blockinfo2 : this.blocks) {
        if (block == null || block != template$blockinfo2.blockState.getBlock()) {
          BlockPos blockpos1 = transformedBlockPos(p_189960_4_, template$blockinfo2.pos).add((Vec3i)p_189960_2_);
          if (structureboundingbox == null || structureboundingbox.isVecInside((Vec3i)blockpos1)) {
  ...
  private void addEntitiesToWorld(World worldIn, BlockPos pos, Mirror mirrorIn, Rotation rotationIn, @Nullable StructureBoundingBox aabb) {
    for (EntityInfo template$entityinfo : this.entities) {
      BlockPos blockpos = transformedBlockPos(template$entityinfo.blockPos, mirrorIn, rotationIn).add((Vec3i)pos);
      if (aabb == null || aabb.isVecInside((Vec3i)blockpos)) {
        Entity entity;
        NBTTagCompound nbttagcompound = template$entityinfo.entityData;
        Vec3d vec3d = transformedVec3d(template$entityinfo.pos, mirrorIn, rotationIn);
        Vec3d vec3d1 = vec3d.addVector(pos.getX(), pos.getY(), pos.getZ());
        NBTTagList nbttaglist = new NBTTagList();
        nbttaglist.appendTag((NBTBase)new NBTTagDouble(vec3d1.xCoord));
        nbttaglist.appendTag((NBTBase)new NBTTagDouble(vec3d1.yCoord));
        nbttaglist.appendTag((NBTBase)new NBTTagDouble(vec3d1.zCoord));
            ...
            if (compound.hasKey("entities", 9)) {
              ...
              for (int i = 0; i < nbttaglist.tagCount(); i++) {
                ...
                if (nbttagcompound.hasKey("nbt", 10))
                  nbttagcompound.setTag("nbt", (NBTBase)fixer.process((IFixType)FixTypes.ENTITY, nbttagcompound.getCompoundTag("nbt"), versionIn)); 
              } 
            } 
            if (compound.hasKey("blocks", 9)) {
              ...
              for (int j = 0; j < nbttaglist1.tagCount(); j++) {
                ...
                if (nbttagcompound1.hasKey("nbt", 10))
                  nbttagcompound1.setTag("nbt", (NBTBase)fixer.process((IFixType)FixTypes.BLOCK_ENTITY, nbttagcompound1.getCompoundTag("nbt"), versionIn)); 
              } 
            } 
            return compound;
          }
  ...
  public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
    BasicPalette template$basicpalette = new BasicPalette(null);
    NBTTagList nbttaglist = new NBTTagList();
    for (BlockInfo template$blockinfo : this.blocks) {
      NBTTagCompound nbttagcompound = new NBTTagCompound();
      nbttagcompound.setTag("pos", (NBTBase)writeInts(new int[] { template$blockinfo.pos.getX(), template$blockinfo.pos.getY(), template$blockinfo.pos.getZ() }));
      ...
      if (template$blockinfo.tileentityData != null)
        nbttagcompound.setTag("nbt", (NBTBase)template$blockinfo.tileentityData); 
      nbttaglist.appendTag((NBTBase)nbttagcompound);
    } 
    NBTTagList nbttaglist1 = new NBTTagList();
    for (EntityInfo template$entityinfo : this.entities) {
      NBTTagCompound nbttagcompound1 = new NBTTagCompound();
      nbttagcompound1.setTag("pos", (NBTBase)writeDoubles(new double[] { template$entityinfo.pos.xCoord, template$entityinfo.pos.yCoord, template$entityinfo.pos.zCoord }));
      nbttagcompound1.setTag("blockPos", (NBTBase)writeInts(new int[] { template$entityinfo.blockPos.getX(), template$entityinfo.blockPos.getY(), template$entityinfo.blockPos.getZ() }));
      if (template$entityinfo.entityData != null)
        nbttagcompound1.setTag("nbt", (NBTBase)template$entityinfo.entityData); 
      nbttaglist1.appendTag((NBTBase)nbttagcompound1);
    } 
    NBTTagList nbttaglist2 = new NBTTagList();
    for (IBlockState iblockstate : template$basicpalette)
      nbttaglist2.appendTag((NBTBase)NBTUtil.writeBlockState(new NBTTagCompound(), iblockstate)); 
    nbt.setTag("palette", (NBTBase)nbttaglist2);
    nbt.setTag("blocks", (NBTBase)nbttaglist);
    nbt.setTag("entities", (NBTBase)nbttaglist1);
    nbt.setTag("size", (NBTBase)writeInts(new int[] { this.size.getX(), this.size.getY(), this.size.getZ() }));
  ...
  public void read(NBTTagCompound compound) {
    ...
    for (int j = 0; j < nbttaglist3.tagCount(); j++) {
      NBTTagCompound nbttagcompound1, nbttagcompound = nbttaglist3.getCompoundTagAt(j);
  ...
  private NBTTagList writeInts(int... values) {
    ...
    byte b;
    int i, arrayOfInt[];
    for (i = (arrayOfInt = values).length, b = 0; b < i; ) {
      int j = arrayOfInt[b];
      nbttaglist.appendTag((NBTBase)new NBTTagInt(j));
      b++;
    } 
    return nbttaglist;
  }
  ...
  private NBTTagList writeDoubles(double... values) {
    ...
    for (i = (arrayOfDouble = values).length, b = 0; b < i; ) {
