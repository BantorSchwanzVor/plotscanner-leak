  ...
  public int findChunksForSpawning(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs, boolean spawnOnSetTickRate) {
    ...
    for (EntityPlayer entityplayer : worldServerIn.playerEntities) {
      if (!entityplayer.isSpectator()) {
        int m = MathHelper.floor(entityplayer.posX / 16.0D);
        int k = MathHelper.floor(entityplayer.posZ / 16.0D);
        int l = 8;
        for (int i1 = -8; i1 <= 8; i1++) {
          for (int j1 = -8; j1 <= 8; j1++) {
            boolean flag = !(i1 != -8 && i1 != 8 && j1 != -8 && j1 != 8);
            ChunkPos chunkpos = new ChunkPos(i1 + m, j1 + k);
                ...
                for (int i4 = 0; i4 < l3; i4++) {
                  ...
                  if (!worldServerIn.isAnyPlayerWithinRangeAt(f, i3, f1, 24.0D) && blockpos1.distanceSq(f, i3, f1) >= 576.0D) {
                    if (biome$spawnlistentry == null) {
                      biome$spawnlistentry = worldServerIn.getSpawnListEntryForTypeAt(enumcreaturetype, (BlockPos)blockpos$mutableblockpos);
                      if (biome$spawnlistentry == null)
                        break; 
                    } 
                    if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biome$spawnlistentry, (BlockPos)blockpos$mutableblockpos) && canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biome$spawnlistentry.entityClass), worldServerIn, (BlockPos)blockpos$mutableblockpos)) {
