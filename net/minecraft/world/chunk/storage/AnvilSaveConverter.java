...
public class AnvilSaveConverter extends SaveFormatOld {
  ...
  public ISaveHandler getSaveLoader(String saveName, boolean storePlayerdata) {
    return (ISaveHandler)new AnvilSaveHandler(this.savesDirectory, saveName, storePlayerdata, this.dataFixer);
  }
  ...
  public boolean convertMapFormat(String filename, IProgressUpdate progressCallback) {
    ...
    if (worldinfo != null && worldinfo.getTerrainType() == WorldType.FLAT) {
      BiomeProviderSingle biomeProviderSingle = new BiomeProviderSingle(Biomes.PLAINS);
    } else {
      biomeprovider = new BiomeProvider(worldinfo);
    } 
    convertFile(new File(file1, "region"), list, biomeprovider, 0, i, progressCallback);
    convertFile(new File(file2, "region"), list1, (BiomeProvider)new BiomeProviderSingle(Biomes.HELL), list.size(), i, progressCallback);
    convertFile(new File(file3, "region"), list2, (BiomeProvider)new BiomeProviderSingle(Biomes.SKY), list.size() + list1.size(), i, progressCallback);
