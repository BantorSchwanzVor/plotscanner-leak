...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
  ...
  public Chunk loadChunk(World worldIn, int x, int z) throws IOException {
    ...
    if (nbttagcompound == null) {
      ...
      if (datainputstream == null)
        return null; 
      nbttagcompound = this.field_193416_e.process((IFixType)FixTypes.CHUNK, CompressedStreamTools.read(datainputstream));
    } 
            ...
            if (compound.hasKey("Level", 10)) {
              ...
              if (nbttagcompound.hasKey("Entities", 9)) {
                ...
                for (int i = 0; i < nbttaglist.tagCount(); i++)
                  nbttaglist.set(i, (NBTBase)fixer.process((IFixType)FixTypes.ENTITY, (NBTTagCompound)nbttaglist.get(i), versionIn)); 
              } 
              if (nbttagcompound.hasKey("TileEntities", 9)) {
                NBTTagList nbttaglist1 = nbttagcompound.getTagList("TileEntities", 10);
                for (int j = 0; j < nbttaglist1.tagCount(); j++)
                  nbttaglist1.set(j, (NBTBase)fixer.process((IFixType)FixTypes.BLOCK_ENTITY, (NBTTagCompound)nbttaglist1.get(j), versionIn)); 
              } 
            } 
            return compound;
          }
  ...
  private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound compound) {
    ...
    for (j = (arrayOfExtendedBlockStorage1 = aextendedblockstorage).length, b = 0; b < j; ) {
    ...
    for (int i = 0; i < (chunkIn.getEntityLists()).length; i++) {
      for (Entity entity : chunkIn.getEntityLists()[i]) {
        ...
        if (entity.writeToNBTOptional(nbttagcompound2)) {
          ...
          nbttaglist1.appendTag((NBTBase)nbttagcompound2);
        } 
      } 
    } 
    compound.setTag("Entities", (NBTBase)nbttaglist1);
    ...
    compound.setTag("TileEntities", (NBTBase)nbttaglist2);
    List<NextTickListEntry> list = worldIn.getPendingBlockUpdates(chunkIn, false);
    if (list != null) {
      long l = worldIn.getTotalWorldTime();
      ...
      for (NextTickListEntry nextticklistentry : list) {
        ...
        nbttagcompound1.setInteger("t", (int)(nextticklistentry.scheduledTime - l));
        nbttagcompound1.setInteger("p", nextticklistentry.priority);
        nbttaglist3.appendTag((NBTBase)nbttagcompound1);
      } 
      compound.setTag("TileTicks", (NBTBase)nbttaglist3);
    } 
  }
