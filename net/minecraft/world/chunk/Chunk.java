...
public class Chunk {
  ...
  public void onChunkLoad() {
    ...
    for (i = (arrayOfClassInheritanceMultiMap = this.entityLists).length, b = 0; b < i; ) {
      ClassInheritanceMultiMap<Entity> classinheritancemultimap = arrayOfClassInheritanceMultiMap[b];
      this.worldObj.loadEntities((Collection)classinheritancemultimap);
      b++;
    } 
  }
  ...
  public void onChunkUnload() {
    ...
    for (i = (arrayOfClassInheritanceMultiMap = this.entityLists).length, b = 0; b < i; ) {
      ClassInheritanceMultiMap<Entity> classinheritancemultimap = arrayOfClassInheritanceMultiMap[b];
      this.worldObj.unloadEntities((Collection)classinheritancemultimap);
      b++;
    } 
  }
        ...
        for (Entity entity : this.entityLists[k]) {
          if (entity.getEntityBoundingBox().intersectsWith(aabb) && entity != entityIn) {
            ...
            if (aentity != null) {
              ...
              for (m = (arrayOfEntity = aentity).length, b = 0; b < m; ) {
                Entity entity1 = arrayOfEntity[b];
  ...
  public void onTick(boolean p_150804_1_) {
    ...
    while (!this.tileEntityPosQueue.isEmpty()) {
      ...
      if (getTileEntity(blockpos, EnumCreateEntityType.CHECK) == null && getBlockState(blockpos).getBlock().hasTileEntity()) {
  ...
  private boolean checkLight(int x, int z) {
    ...
    for (int l = blockpos$mutableblockpos.getY(); l > 0; l--) {
      ...
      if (getBlockState((BlockPos)blockpos$mutableblockpos).getLightValue() > 0)
