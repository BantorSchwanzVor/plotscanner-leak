...
public class DragonFightManager {
  ...
  private final BossInfoServer bossInfo = (BossInfoServer)(new BossInfoServer((ITextComponent)new TextComponentTranslation("entity.EnderDragon.name", new Object[0]), BossInfo.Color.PINK, BossInfo.Overlay.PROGRESS)).setPlayEndBossMusic(true).setCreateFog(true);
  ...
  public DragonFightManager(WorldServer worldIn, NBTTagCompound compound) {
    ...
    if (compound.hasKey("Gateways", 9)) {
      ...
      this.gateways.addAll((Collection<? extends Integer>)ContiguousSet.create(Range.closedOpen(Integer.valueOf(0), Integer.valueOf(20)), DiscreteDomain.integers()));
      Collections.shuffle(this.gateways, new Random(worldIn.getSeed()));
    } 
    this.portalPattern = FactoryBlockPattern.start().aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " }).aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " }).aisle(new String[] { "       ", "       ", "       ", "   #   ", "       ", "       ", "       " }).aisle(new String[] { "  ###  ", " #   # ", "#     #", "#  #  #", "#     #", " #   # ", "  ###  " }).aisle(new String[] { "       ", "  ###  ", " ##### ", " ##### ", " ##### ", "  ###  ", "       " }).where('#', BlockWorldState.hasState((Predicate)BlockMatcher.forBlock(Blocks.BEDROCK))).build();
  }
  ...
  public NBTTagCompound getCompound() {
    ...
    if (this.exitPortalLocation != null)
      nbttagcompound.setTag("ExitPortalLocation", (NBTBase)NBTUtil.createPosTag(this.exitPortalLocation)); 
    NBTTagList nbttaglist = new NBTTagList();
    Iterator<Integer> iterator = this.gateways.iterator();
    while (iterator.hasNext()) {
      int i = ((Integer)iterator.next()).intValue();
      nbttaglist.appendTag((NBTBase)new NBTTagInt(i));
    } 
    nbttagcompound.setTag("Gateways", (NBTBase)nbttaglist);
  ...
  private BlockPattern.PatternHelper findExitPortal() {
    for (int i = -8; i <= 8; i++) {
      for (int j = -8; j <= 8; j++) {
        ...
        for (TileEntity tileentity : chunk.getTileEntityMap().values()) {
          if (tileentity instanceof net.minecraft.tileentity.TileEntityEndPortal) {
            BlockPattern.PatternHelper blockpattern$patternhelper = this.portalPattern.match((World)this.world, tileentity.getPos());
    ...
    for (int l = k; l >= 0; l--) {
      BlockPattern.PatternHelper blockpattern$patternhelper1 = this.portalPattern.match((World)this.world, new BlockPos(WorldGenEndPodium.END_PODIUM_LOCATION.getX(), l, WorldGenEndPodium.END_PODIUM_LOCATION.getZ()));
  ...
  private void findAliveCrystals() {
    ...
    for (i = (arrayOfEndSpike = BiomeEndDecorator.getSpikesForWorld((World)this.world)).length, b = 0; b < i; ) {
      WorldGenSpikes.EndSpike worldgenspikes$endspike = arrayOfEndSpike[b];
  ...
  private void spawnNewGateway() {
    if (!this.gateways.isEmpty()) {
      ...
      int j = (int)(96.0D * Math.cos(2.0D * (-3.141592653589793D + 0.15707963267948966D * i)));
      int k = (int)(96.0D * Math.sin(2.0D * (-3.141592653589793D + 0.15707963267948966D * i)));
  ...
  private void generateGateway(BlockPos pos) {
    ...
    (new WorldGenEndGateway()).generate((World)this.world, new Random(), pos);
  }
  ...
  private void generatePortal(boolean active) {
    ...
    worldgenendpodium.generate((World)this.world, new Random(), this.exitPortalLocation);
  }
  
  private EntityDragon func_192445_m() {
    this.world.getChunkFromBlockCoords(new BlockPos(0, 128, 0));
    EntityDragon entitydragon = new EntityDragon((World)this.world);
    ...
    this.world.spawnEntityInWorld((Entity)entitydragon);
    this.dragonUniqueId = entitydragon.getUniqueID();
    return entitydragon;
  }
  ...
  public void onCrystalDestroyed(EntityEnderCrystal crystal, DamageSource dmgSrc) {
    if (this.respawnState != null && this.crystals.contains(crystal)) {
      ...
      if (entity instanceof EntityDragon)
        ((EntityDragon)entity).onCrystalDestroyed(crystal, new BlockPos((Entity)crystal), dmgSrc); 
    } 
  }
