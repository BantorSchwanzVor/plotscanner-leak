...
public class ItemArmorStand extends Item {
  ...
  public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
    ...
    boolean flag1 = (!playerIn.isAirBlock(blockpos) && !playerIn.getBlockState(blockpos).getBlock().isReplaceable((IBlockAccess)playerIn, blockpos));
    int i = flag1 | ((!playerIn.isAirBlock(blockpos1) && !playerIn.getBlockState(blockpos1).getBlock().isReplaceable((IBlockAccess)playerIn, blockpos1)) ? 1 : 0);
    if (i != 0)
      return EnumActionResult.FAIL; 
    ...
    if (!playerIn.isRemote) {
      ...
      ItemMonsterPlacer.applyItemEntityDataToEntity(playerIn, stack, itemstack, (Entity)entityarmorstand);
      playerIn.spawnEntityInWorld((Entity)entityarmorstand);
