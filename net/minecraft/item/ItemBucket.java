...
public class ItemBucket extends Item {
  ...
  public ActionResult<ItemStack> onItemRightClick(World itemStackIn, EntityPlayer worldIn, EnumHand playerIn) {
    ...
    if (flag) {
      ...
      if (material == Material.WATER && ((Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL)).intValue() == 0) {
        ...
        return new ActionResult(EnumActionResult.SUCCESS, fillBucket(itemstack, worldIn, Items.WATER_BUCKET));
      } 
      if (material == Material.LAVA && ((Integer)iblockstate.getValue((IProperty)BlockLiquid.LEVEL)).intValue() == 0) {
        ...
        itemStackIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 11);
        worldIn.addStat(StatList.getObjectUseStats(this));
        return new ActionResult(EnumActionResult.SUCCESS, fillBucket(itemstack, worldIn, Items.LAVA_BUCKET));
      } 
      return new ActionResult(EnumActionResult.FAIL, itemstack);
    } 
    boolean flag1 = itemStackIn.getBlockState(blockpos).getBlock().isReplaceable((IBlockAccess)itemStackIn, blockpos);
    BlockPos blockpos1 = (flag1 && raytraceresult.sideHit == EnumFacing.UP) ? blockpos : blockpos.offset(raytraceresult.sideHit);
    if (!worldIn.canPlayerEdit(blockpos1, raytraceresult.sideHit, itemstack))
      return new ActionResult(EnumActionResult.FAIL, itemstack); 
    if (tryPlaceContainedLiquid(worldIn, itemStackIn, blockpos1)) {
  ...
  public boolean tryPlaceContainedLiquid(@Nullable EntityPlayer player, World worldIn, BlockPos posIn) {
    ...
    boolean flag1 = iblockstate.getBlock().isReplaceable((IBlockAccess)worldIn, posIn);
