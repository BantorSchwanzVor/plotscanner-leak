...
public class ItemDye extends Item {
  ...
  public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
    ...
    if (enumdyecolor == EnumDyeColor.WHITE) {
      ...
      if (block == Blocks.LOG && iblockstate.getValue((IProperty)BlockOldLog.VARIANT) == BlockPlanks.EnumType.JUNGLE) {
        if (hand == EnumFacing.DOWN || hand == EnumFacing.UP)
          return EnumActionResult.FAIL; 
        worldIn = worldIn.offset(hand);
        if (playerIn.isAirBlock(worldIn)) {
          IBlockState iblockstate1 = Blocks.COCOA.onBlockPlaced(playerIn, worldIn, hand, facing, hitX, hitY, 0, (EntityLivingBase)stack);
  ...
  public static void spawnBonemealParticles(World worldIn, BlockPos pos, int amount) {
    ...
    if (iblockstate.getMaterial() != Material.AIR)
      for (int i = 0; i < amount; i++) {
        ...
        worldIn.spawnParticle(EnumParticleTypes.VILLAGER_HAPPY, (pos.getX() + itemRand.nextFloat()), pos.getY() + itemRand.nextFloat() * (iblockstate.getBoundingBox((IBlockAccess)worldIn, pos)).maxY, (pos.getZ() + itemRand.nextFloat()), d0, d1, d2, new int[0]);
      }  
  }
