...
public class SPacketChunkData implements Packet<INetHandlerPlayClient> {
  ...
  public SPacketChunkData(Chunk p_i47124_1_, int p_i47124_2_) {
    ...
    this.buffer = new byte[calculateChunkSize(p_i47124_1_, flag, p_i47124_2_)];
    this.availableSections = extractChunkData(new PacketBuffer(getWriteBuffer()), p_i47124_1_, flag, p_i47124_2_);
    this.tileEntityTags = Lists.newArrayList();
    for (Map.Entry<BlockPos, TileEntity> entry : (Iterable<Map.Entry<BlockPos, TileEntity>>)p_i47124_1_.getTileEntityMap().entrySet()) {
