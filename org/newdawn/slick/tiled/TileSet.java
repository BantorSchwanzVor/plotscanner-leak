...
public class TileSet {
  ...
  public TileSet(TiledMap map, Element element, boolean loadImage) throws SlickException {
    ...
    if (source != null && !source.equals(""))
      try {
        InputStream in = ResourceLoader.getResourceAsStream(
            String.valueOf(map.getTilesLocation()) + "/" + source);
    ...
    if (loadImage) {
      Image image = new Image(String.valueOf(map.getTilesLocation()) + "/" + ref, false, 
          2, trans);
  ...
  public void setTileSetImage(Image image) {
    this.tiles = new SpriteSheet(image, this.tileWidth, this.tileHeight, this.tileSpacing, 
        this.tileMargin);
