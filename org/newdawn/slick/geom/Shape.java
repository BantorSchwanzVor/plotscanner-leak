...
public abstract class Shape implements Serializable {
  ...
  public boolean contains(float x, float y) {
    ...
    for (int i = 0; i < npoints; i += 2) {
      ...
      if (((xnew < x) ? true : false) == ((x <= xold) ? true : false) && (
        y - y1) * (x2 - x1) < (
        y2 - y1) * (x - x1))
        result = !result; 
      xold = xnew;
  ...
  protected void calculateRadius() {
    ...
    for (int i = 0; i < this.points.length; i += 2) {
      float temp = (this.points[i] - this.center[0]) * (this.points[i] - this.center[0]) + (
        this.points[i + 1] - this.center[1]) * (this.points[i + 1] - this.center[1]);
